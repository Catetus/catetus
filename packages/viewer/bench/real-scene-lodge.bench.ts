...
export async function main(): Promise<void> {
  ...
  // WebGPU 1.0 limits dispatchWorkgroups to 65535 per dimension. At
  // workgroup_size=256, that's 65535 * 256 = 16 776 960 splats per single
  // dispatch.
  //
  // Per-splat compute kernels (decode, project, gather, cull, lod_blend,
  // tile_bin, wsr_accumulate) carve large dispatches into <= 65535-wg chunks
  // via packages/viewer/src/webgpu/multi-dispatch.ts (Stage 1-4,
  // feat/wgpu-multidispatch).
  //
  // The radix sort (packages/viewer/src/webgpu/radix_sort.ts) now chunks
  // sorts that exceed 16.7M splats via per-chunk sort + binary merge tree
