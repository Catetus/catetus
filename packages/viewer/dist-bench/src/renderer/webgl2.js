...
export class WebGL2Renderer {
    ...
    async init(opts) {
        ...
        if (!gl)
            throw new Error('renderer_unavailable: webgl2 unsupported');
        this.gl = gl;
        ...
        if (!qb)
            throw new Error('renderer_init_failed: createBuffer (quad)');
        this.quadBuffer = qb;
        ...
        if (!ib)
            throw new Error('renderer_init_failed: createBuffer (instance)');
        this.instanceBuffer = ib;
        // VAO binds both buffers + divisors.
        const vao = gl.createVertexArray();
        if (!vao)
            throw new Error('renderer_init_failed: createVertexArray');
        this.vao = vao;
    ...
    async renderFrame(camera) {
        ...
        if (this.uViewportSize)
            gl.uniform2f(this.uViewportSize, width, height);
    ...
    linkProgram(gl, vs, fs) {
        const compile = (type, src) => {
            ...
            if (!sh)
                throw new Error('renderer_init_failed: createShader');
        ...
        if (!program)
            throw new Error('renderer_init_failed: createProgram');
