...
Browser viewer SDK for Catetus optimized glTF / SPZ gaussian-splat assets.
Implements [SPEC-0008](../../specs/0008-viewer-sdk.md).

## Install

```bash
pnpm add @catetus/viewer
```
...
The package ships an HTML shell consumed by the CLI's `catetus preview`
...
Resolved path: `@catetus/viewer/preview-shell.html`.

## Scripts

```bash
pnpm --filter @catetus/viewer build   # tsc -> dist/
pnpm --filter @catetus/viewer lint    # tsc --noEmit
pnpm --filter @catetus/viewer test    # vitest run
```
