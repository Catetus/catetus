...
/** Options for [`loadSogContainer`] — companion files + URL-load auto-fetch. */
export interface LoadSogOpts {
  /** Map of sidecar filename → bytes. The only key the SOG loader honors
   *  is `<sogname>.v5tail` (any *.v5tail in the bag is a fallback match). */
...
export async function loadSogContainer(
  buf: Uint8Array,
  sourceName: string,
  opts: LoadSogOpts = {},
): Promise<SplatScene> {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const [w, x, y, z] = unpackQuat(
      quats.rgba[o + 0]!, quats.rgba[o + 1]!, quats.rgba[o + 2]!, tag,
    );
