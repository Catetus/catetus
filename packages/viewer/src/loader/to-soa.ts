...
import {
  ...
  type SoaAttributeLayout,
} from '../manifest.js';
import { SOA_BYTES_PER_SPLAT } from '../progressive/uploader.js';
import { shRestCoefCount, type SplatScene } from './splat-scene.js';
...
/** Output of {@link splatSceneToSoaChunk}: descriptor + ready-to-upload bytes. */
export interface SoaChunkOutput {
 ...
 * `scene.count * 56`.
 */
export function splatSceneToSoaChunk(
  scene: SplatScene,
  uri = 'scene:in-memory',
): SoaChunkOutput {
  const N = scene.count;
  ...
  if (shDegree === 0 && dcEnd !== N * SOA_BYTES_PER_SPLAT) {
    throw new Error(
      `to-soa: stride bookkeeping mismatch (computed ${dcEnd} B, expected ${N * SOA_BYTES_PER_SPLAT} B)`,
    );
  }
