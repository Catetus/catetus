...
/** SOG v2 meta.json shape. */
export interface SogMetaV2 {
  version: 2;
  count: number;
...
/** Per-splat values in the canonical viewer domain. */
export interface SogSplat {
  /** World-space position (post-`invLogTransform`). */
  position: [number, number, number];
  /** Linear scale (post-`exp` from raw log-scale). */
  scale: [number, number, number];
  /** Quaternion (w, x, y, z) — same order the splat-transform writer emits. */
  rotation: [number, number, number, number];
  /** Opacity in [0,1] (post-sigmoid). */
  opacity: number;
  /** SH DC coefficient — passed straight through as RGB the way the viewer
   *  treats every other source (PLY's raw `f_dc_n`). */
...
export interface SogScene {
  splatCount: number;
  splats: SogSplat[];
  /** Axis-aligned bbox over the decoded positions. */
  bbox: { min: [number, number, number]; max: [number, number, number] };
}

/** Pluggable WebP decoder — Node and browser environments supply different
 *  implementations. Must return tightly-packed RGBA8 + dimensions. */
export interface WebPDecoder {
  decode(bytes: Uint8Array): Promise<{ rgba: Uint8Array; width: number; height: number }>;
}

/** File system bundling the SOG members (decoded ZIP entries or a directory). */
export interface SogFileSystem {
...
function unpackQuat(
  ...
  const idxTable: [number, number, number][] = [
    [1, 2, 3],
    [0, 2, 3],
    [0, 1, 3],
    [0, 1, 2],
...
function decodeMeans16(lo: Uint8Array, hi: Uint8Array, count: number) {
 ...
 */
export async function readSog(
  fs: SogFileSystem,
  metaName: string,
  decoder: WebPDecoder,
): Promise<SogScene> {
  const metaBytes = await fs.read(metaName);
  const meta = JSON.parse(new TextDecoder().decode(metaBytes)) as SogMetaV2 | { version?: number };
  if (meta.version !== 2) {
    throw new Error(
      `sog_unsupported_version: SOG meta version ${String(meta.version)} (only v2 supported)`,
    );
  }
  const m = meta as SogMetaV2;
  const count = m.count;
  const splats: SogSplat[] = new Array(count);
  ...
  // SH_C0 offset because that's how `catetus-ply` reads PLY today. We
...
export function sogSceneToGltf(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (s.position[0] < pxMin) pxMin = s.position[0];
    if (s.position[1] < pyMin) pyMin = s.position[1];
    if (s.position[2] < pzMin) pzMin = s.position[2];
    if (s.position[0] > pxMax) pxMax = s.position[0];
    if (s.position[1] > pyMax) pyMax = s.position[1];
    if (s.position[2] > pzMax) pzMax = s.position[2];
  ...
  // directly, not re-quantized through Catetus's own optimize step.
  ...
  const gltfDoc = {
    asset: { version: '2.0', generator: 'catetus sog-loader' },
