/**
 * `tileset.json` fetcher + tree builder for SplatForge streaming tiles.
 *
 * Consumes the 3D Tiles 1.1 hierarchy emitted by
 * `splatforge-optimize::tileset::write_tileset` and produces an in-memory
...
export interface TileNode {
  ...
  /** Refinement mode — `"REPLACE"` is the only mode SplatForge emits. */
  refine: 'REPLACE' | 'ADD';
