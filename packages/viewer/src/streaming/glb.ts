...
/* ------------------------------------------------------------------------ */
/* Catetus SOG primitives: SF_zstd_split_buffer + SF_gaussian_splatting_palette */
/*                                                                          */
/* These mirror the bench-harness reference decoder in                       */
/* `experiments/w3-fidelity-harness/code/cpu-fidelity.mjs`. They are pure-JS */
/* (no Node imports), and take a `zstdDecompress` callback so the viewer    */
/* package stays dependency-free — the caller wires in `fzstd.decompress`   */
/* (or any compatible decoder).                                             */
/* ------------------------------------------------------------------------ */

/** Function signature shared by `fzstd.decompress` and Node's zstd. */
 ...
 * @param zstdDecompress Pure zstd frame decoder (e.g. `fzstd.decompress`).
 */
export function decodeShPaletteSidecar(
  compressed: Uint8Array,
  ext: ShPaletteExt | null,
  zstdDecompress: ZstdDecompress,
): ShPalette {
  ...
  const version = dv.getUint32(4, true);
  if (version !== 1) throw new Error(`unsupported .shpal version: ${version}`);
