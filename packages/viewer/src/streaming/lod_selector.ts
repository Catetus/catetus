 ...
 * the screen.
 *
 * Refinement rule for `refine: "REPLACE"` (the only mode SplatForge emits):
...
/** Output of {@link selectVisibleTiles}. */
export interface TileSelection {
  /** Tiles the renderer should draw this frame. Coarse → fine order. */
  render: TileNode[];
  /** Tiles the streamer should fetch (if not already resident) so we can
   *  refine into them on a future frame. Sorted by SSE descending so the
   *  biggest pixel-error wins come in first. */
  fetch: TileNode[];
}

/** Knobs for {@link selectVisibleTiles}. */
export interface SelectorOptions {
  /** Eye position in world space. */
  eye: [number, number, number];
  /** Camera vertical FOV, radians. */
  fovY: number;
  /** Viewport height, pixels. */
  viewportHeight: number;
  /** Cesium SSE cutoff. Defaults to 16. */
  maximumScreenSpaceError?: number;
  /** Frustum planes. Tiles outside the frustum are culled before LOD. */
  frustum: Frustum;
  /**
   * Optional set of "resident" tile IDs — tiles whose content is loaded and
   * ready to render. When provided, the selector falls back to the
   * deepest-resident ancestor for any tile that hasn't loaded yet, keeping
   * the screen filled while children stream in (Cesium's "skipLODs = false"
   * behavior).
   */
  resident?: Set<string>;
}
...
export function selectVisibleTiles(
  ...
  const fetch: TileNode[] = [];
  // Map of tile -> sse for sort. We collect during the walk and sort once.
  const fetchSse: Map<TileNode, number> = new Map();

  const visit = (tile: TileNode): void => {
    if (!aabbIntersectsFrustum(tile.aabb, opts.frustum)) return;
    ...
    if (shouldRefine) {
      ...
      if (allChildrenResident || !opts.resident) {
        for (const c of tile.children) visit(c);
      } else {
 ...
 */
function pushRenderable(
  tile: TileNode,
  render: TileNode[],
  fetch: TileNode[],
  fetchSse: Map<TileNode, number>,
  sse: number,
  resident?: Set<string>,
): void {
  if (!resident) {
  ...
  let cur: TileNode | undefined = tile;
