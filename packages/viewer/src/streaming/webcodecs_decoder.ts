 ...
 * `catetus-private/docs/codecgs-lite-decision.md` for the
...
// (this is a sketch — most symbols are unused until we wire it up)

/** WebCodecs handles for the lifetime of a tileset load. */
export interface CodecGsLiteSession {
  /** One configured decoder. Reused across channels via reset(). */
  decoder: VideoDecoder;
  /** Codec string passed to `VideoDecoder.configure`. */
  codec: string;
}

/** Parsed tile header (24 bytes) + per-channel descriptors. */
export interface CodecGsLiteTile {
  version: number;
  channelCount: number;
  splatCount: number;
  planeWidth: number;
  ...
  /** 0 = AV1, 1 = HEVC. */
  codecId: number;
  /** Per-channel dequantisation params + bitstream pointers. */
  channels: readonly CodecGsLiteChannel[];
  /** The full file buffer; channels reference into this by offset. */
  buffer: ArrayBuffer;
}

export interface CodecGsLiteChannel {
  /** Min/max for u8 → float32 dequantisation. */
  min: number;
  max: number;
  /** Byte offset into the tile buffer. */
  byteOffset: number;
  /** Byte length of the still-image bitstream. */
  byteLength: number;
}
 ...
 */
export async function probeCodecGsLiteSupport(
  codecId: 0 | 1 = 0,
): Promise<{ codec: string } | null> {
...
export function parseCodecGsLiteHeader(buf: ArrayBuffer): CodecGsLiteTile {
  ...
  const channels: CodecGsLiteChannel[] = [];
  const descBase = 24;
 ...
 * ships green.
 */
export async function decodeCodecGsLiteTile(
  tile: CodecGsLiteTile,
  ...
  let resolveNext: ((frame: VideoFrame) => void) | null = null;
  ...
  try {
    for (let c = 0; c < tile.channelCount; c++) {
      ...
      const framePromise = new Promise<VideoFrame>((res) => {
        resolveNext = res;
      });
 ...
 */
export function unsortDecodedTile(
  decoded: Float32Array,
  tile: CodecGsLiteTile,
  inversePermutation: Uint32Array,
): Float32Array {
 ...
 */
export function estimateDecodeMs(tile: CodecGsLiteTile): number {
