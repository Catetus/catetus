/**
 * Chunk loader. Streams binary tiles in priority order, validating checksums
 * when `SubtleCrypto` is available.
 *
 * BLAKE3 is not implemented natively in browsers; we degrade to a SHA-256
 * fingerprint when checksums are present and `SubtleCrypto.digest` exists.
 * If neither is available we emit a `warning` and skip validation entirely.
 */
import type { ChunkDescriptor } from './manifest.js';

/** A loaded chunk paired with its descriptor. */
export interface LoadedChunk {
  descriptor: ChunkDescriptor;
  bytes: Uint8Array;
}

/** Result of `validateChecksum`. */
...
export async function fetchChunkBytes(
  ...
  try {
    ...
    throw new Error(`chunk_fetch_failed: ${(err as Error).message}`);
  }
...
export async function validateChecksum(
  ...
  try {
    ...
    if (hex === expected.toLowerCase()) return { ok: true };
    return { ok: false, reason: 'unsupported' };
  } catch {
    return { ok: false, reason: 'unsupported' };
  }
