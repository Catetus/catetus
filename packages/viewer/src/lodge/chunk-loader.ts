...
/** Fetch interface — overridable for tests + non-browser embeddings. */
export type ChunkFetcher = (url: string) => Promise<Uint8Array>;

/** Pipeline surface this loader requires. Matches the relevant parts of
 *  `ComputeDecodePipeline` so tests can substitute a mock. */
export interface UploadablePipeline {
  uploadChunk(descriptor: ChunkDescriptor, bytes: Uint8Array): void;
  readonly splatCount: number;
  readonly capacity: number;
}

/** LodgeChunkLoader construction options. */
export interface LodgeChunkLoaderOptions {
  /** Base URL the chunk paths are resolved against. Trailing `/`
   *  optional. */
  baseUrl: string;
  /** GPU pipeline that ingests decoded chunks. */
  pipeline: UploadablePipeline;
  /** Optional fetcher override (defaults to {@link fetch}). */
  fetcher?: ChunkFetcher;
  /** LRU byte budget. Default 8 GiB. */
  memoryBudgetBytes?: number;
}

/** Snapshot returned by {@link LodgeChunkLoader.loadLevel}. */
export interface LoadLevelResult {
  /** Level loaded. */
  level: number;
  /** Number of chunks actually uploaded this call (cache misses). */
  uploadedChunks: number;
  /** Number of chunks served from cache (already resident). */
  cachedChunks: number;
  /** Total splats now decoded into the GPU pipeline at this level. */
  splats: number;
  /** Bytes currently in the loader's LRU. */
  memoryUsedBytes: number;
  /** Wall-clock ms spent inside `loadLevel`. */
  elapsedMs: number;
}

interface CachedEntry {
  /** Stable cache key — `level/chunkIndex`. */
  key: string;
  /** Decoded SoA bytes (kept resident for re-upload after eviction). */
  bytes: Uint8Array;
  /** Splat count for fast budget math. */
  splatCount: number;
  /** Decoded byteLength — same as `bytes.byteLength`. */
  byteLength: number;
}

const DEFAULT_BUDGET = 8 * 1024 * 1024 * 1024; // 8 GiB

const DEFAULT_FETCHER: ChunkFetcher = async (url) => {
...
export class LodgeChunkLoader {
  ...
  static async load(opts: LodgeChunkLoaderOptions): Promise<LodgeChunkLoader> {
    const url = resolveChunkUri(
   ...
   */
  pickLevel(cameraPos: Vec3): number {
   ...
   */
  async loadLevel(level: number, cameraPos?: Vec3): Promise<LoadLevelResult> {
    const t0 = nowMs();
  ...
  async fetchAndDecode(
    chunk: LodgeChunkDescriptor,
  ): Promise<{ bytes: Uint8Array; splatCount: number }> {
    ...
    if (decoded.splatCount !== chunk.splatCount) {
      throw new Error(
        `lodge_chunk_count_mismatch: ${chunk.path} expected ${chunk.splatCount}, got ${decoded.splatCount}`,
      );
    }
   ...
   */
  private evictToBudget(preserveKey: string): void {
    if (this.memoryUsedBytes <= this.memoryBudgetBytes) return;
    // Map insertion order is iteration order; the OLDEST entry is first.
    for (const [key, entry] of this.lru) {
      if (this.memoryUsedBytes <= this.memoryBudgetBytes) break;
...
function cacheKey(level: number, chunkIndex: number): string {
 ...
 */
function resetPipeline(p: UploadablePipeline): void {
  const target = p as unknown as { decodedSplats?: number; chunks?: unknown[] };
