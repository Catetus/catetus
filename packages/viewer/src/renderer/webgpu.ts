...
export interface WebGPURendererOptions {
  ...
  /**
   * Multiplier applied to the first chunk's splat count when sizing the
   * compute pipeline (only used when `computeCapacity` is not explicitly
   * set). Defaults to 1 — sized exactly to the chunk so we don't trip
   * `ComputeDecodePipeline`'s multi-page guard (a scene over ~125K splats
   * with the previous 8× headroom would force multi-page splats which
   * require useFusedProject; see Phase 1 smoke RESULT.md).
   *
   * Streaming callers that grow capacity over multiple chunks should pass
   * an explicit `computeCapacity` instead of relying on this multiplier.
   */
  computeCapacityGrowthFactor?: number;
}
...
export class WebGPURenderer implements Renderer {
  ...
  async init(opts: RendererInitOptions): Promise<void> {
    ...
    const limits = (adapter as unknown as { limits: GPUSupportedLimits }).limits;
   ...
   */
  async _debugReadInstances(n = 2000): Promise<{ clipPos: [number, number, number, number]; cov: [number, number, number, number]; color: [number, number, number, number] }[]> {
    if (!this.device) throw new Error('debug: device missing');
    if (!this.compute) throw new Error('debug: compute pipeline inactive (need useComputeDecode=true)');
    const page = this.compute.instancePages[0];
    if (!page) throw new Error('debug: no instance pages');
    const FLOATS = 12;
    ...
    readback.unmap();
    readback.destroy();
    const out: { clipPos: [number, number, number, number]; cov: [number, number, number, number]; color: [number, number, number, number] }[] = [];
    for (let i = 0; i < count; i++) {
      const o = i * FLOATS;
      out.push({
