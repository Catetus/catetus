...
export interface CullPipelines {
  ...
  projectCmpct: GPUComputePipeline;
  scanPerWg: GPUComputePipeline;
  scanBlockSums: GPUComputePipeline;
  scanAddBlockSums: GPUComputePipeline;
  cullBgl: GPUBindGroupLayout;
  compactBgl: GPUBindGroupLayout;
  projectCmpctBgl: GPUBindGroupLayout;
...
export function createCullPipelines(device: GPUDevice): CullPipelines {
  ...
  const mk = (mod: GPUShaderModule, bgl: GPUBindGroupLayout, entryPoint: string): GPUComputePipeline =>
...
export interface CullPipelineInit {
  device: GPUDevice;
  capacity: number;
  pipes: CullPipelines;
  splatsBuffer: GPUBuffer;
  /** Sort key buffer (caller-owned). cs_project_cmpct writes here. */
  keysBuffer: GPUBuffer;
  /** Sort value buffer (caller-owned). cs_project_cmpct writes here. */
  valuesBuffer: GPUBuffer;
  /** Per-instance Instance record buffer (caller-owned). cs_project_cmpct writes here. */
  instBuffer: GPUBuffer;
  /** Shared project uniforms (re-used by cs_project_cmpct). Host writes here. */
  projectUniforms: GPUBuffer;
}

export class CullPipeline {
  readonly device: GPUDevice;
  readonly capacity: number;
  readonly pipes: CullPipelines;
  readonly flagBuffer: GPUBuffer;
  readonly prefixBuffer: GPUBuffer;
  readonly compactBuffer: GPUBuffer;
  readonly blockSums: GPUBuffer;
  readonly scanUniforms: GPUBuffer;
  readonly cullUniforms: GPUBuffer;
  readonly compactUniforms: GPUBuffer;
  ...
  private readonly projectCmpctBindGroup: GPUBindGroup;
  ...
  private lastSplatCount = 0;

  constructor(init: CullPipelineInit) {
  ...
  encode(
    ...
    // 1. Cull uniforms.
    {
      ...
      f.set(view, 0);
      f.set(viewProj, 16);
      f[32] = viewport[0]; f[33] = viewport[1];
  ...
  async readSurvivorCount(): Promise<number> {
    if (this.lastSplatCount === 0) return 0;
