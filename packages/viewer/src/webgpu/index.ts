...
function shCoefCountForDegree(deg: number): number {
...
export class ComputeDecodePipeline {
  ...
  private projectBindGroup: GPUBindGroup | null;
  ...
  private projectGatherBindGroup: GPUBindGroup | null;
  /** Layouts kept around so Phase 2b can rebuild bind groups after the
   *  SH-rest buffer is replaced on first SH-bearing chunk. */
  private readonly _projectGatherBglSingle: GPUBindGroupLayout | null;
  private _projectGatherBglPaged: GPUBindGroupLayout | null = null;
   ...
   */
  private _rebuildShRestBindGroups(): void {
  ...
  uploadChunk(descriptor: ChunkDescriptor, bytes: Uint8Array): void {
    ...
    if (layout.shRest && layout.shDegree && layout.shDegree > 0) {
      ...
      if (this.shDegree === 0) {
        ...
        throw new Error(
          `compute-decode: SH degree mismatch across chunks ` +
