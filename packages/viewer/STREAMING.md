...
(see `crates/catetus-optimize/src/tileset.rs`).
...
The producer side ships in `catetus-optimize`:

```bash
catetus optimize --preset geospatial input.ply -o /tmp/asset
```
...
See `crates/catetus-optimize/src/tileset.rs` for the full producer
specification.
