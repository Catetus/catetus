 ...
 *
 * Accepts binary little-endian PLY whose `vertex` element carries the canonical
 * 3DGS properties. The bare minimum:
 *
 *   x, y, z, opacity,
 ...
 *
 * Optional: `f_rest_0..f_rest_44` (SH degree 3) — when all 45 are present we
 * carry them through to the renderer as `shRest`. Inria PLY stores SH-rest
 * channel-major: `f_rest_0..14`=R, `f_rest_15..29`=G, `f_rest_30..44`=B. We
 * transpose into the canonical splat-major then coef-major then channel-major
 * layout: `shRest[i*45 + k*3 + c]`.
 *
 * `nx/ny/nz` are tolerated and ignored.
 *
 * Returns a fully normalized {@link SplatScene}.
 */
import {
  computeBbox,
  normalizeQuatInto,
...
interface PlyHeader {
  vertexCount: number;
  /** Per-property record. `byteOffset` is from the start of one vertex record. */
  props: { name: string; type: PlyType; byteOffset: number; size: number }[];
  vertexStride: number;
  bodyOffset: number;
...
function normalizeType(t: string): PlyType {
  switch (t) {
    case 'float32': case 'float': return 'float';
    case 'float64': case 'double': return 'double';
    case 'uint8': case 'uchar': return 'uchar';
    case 'int8': case 'char': return 'char';
...
function parseHeader(buf: Uint8Array): PlyHeader {
  const text = new TextDecoder('ascii').decode(buf.subarray(0, Math.min(buf.length, 1 << 16)));
  const endMarker = '\nend_header\n';
  const endIdx = text.indexOf(endMarker);
  if (endIdx < 0) throw new Error('ply: missing end_header');
  ...
  let format: 'binary_little_endian' | 'binary_big_endian' | 'ascii' | null = null;
  let inVertex = false;
  let vertexCount = 0;
  const props: PlyHeader['props'] = [];
  let stride = 0;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line.startsWith('format ')) {
      ...
      else if (parts[1] === 'ascii') format = 'ascii';
      else throw new Error(`ply: unknown format "${parts[1]}"`);
      continue;
    }
    ...
    if (line.startsWith('property ')) {
      ...
      const parts = line.split(/\s+/);
      if (parts[1] === 'list') {
        // List properties not supported in 3DGS PLY but skip gracefully.
...
function getter(type: PlyType): (dv: DataView, off: number) => number {
  switch (type) {
 ...
 */
export function isLikely3DGSPly(buf: Uint8Array): boolean {
...
export function loadPly(buf: Uint8Array, sourceName: string): SplatScene {
  ...
  for (const r of required) {
    if (!byName.has(r)) throw new Error(`ply: missing required property "${r}"`);
  }
  ...
  const r = (name: string): { get: (dv: DataView, o: number) => number; off: number } => {
  ...
  let shDegree = 0;
  if (byName.has('f_rest_44')) shDegree = 3;
  else if (byName.has('f_rest_23')) shDegree = 2;
  else if (byName.has('f_rest_8')) shDegree = 1;
  const coefCount = shRestCoefCount(shDegree);
  let restAccessors: { get: (dv: DataView, o: number) => number; off: number }[] | null = null;
  ...
  if (shDegree > 0) {
    ...
    if (restAccessors) shRest = new Float32Array(N * total);
  }
  ...
  for (let i = 0; i < N; i++) {
    ...
    // DC color = 0.5 + SH_C0 * f_dc, clamped.
