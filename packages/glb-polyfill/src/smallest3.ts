...
export function decodeSmallest3Quat(packed: number, componentBits: number): [number, number, number, number] {
  ...
  const dq = (q: number) => (q / levels - 0.5) * sqrt2;
  const a = dq(q0);
  const b = dq(q1);
  const c = dq(q2);
 ...
 */
export function decodeSmallest3QuatBuffer(
  packed: Uint32Array,
...
export interface QuatSmallest3Ext {
  componentBits?: number;
  componentType?: number; // 5125 (UINT) per the producer
  layout?: string; // "q0|q1|q2|tag"
  tagBits?: number; // 2
}
