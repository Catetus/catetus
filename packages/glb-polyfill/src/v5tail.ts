...
const MAGIC = [0x53, 0x46, 0x56, 0x35, 0x31, 0x54, 0x41, 0x4c] as const;
/**
 ...
 */
export function decodeV5TailBytes(
  bytes: Uint8Array,
  ...
  for (let i = 0; i < MAGIC.length; i++) {
    if (bytes[i] !== MAGIC[i]) {
      throw new Error(
        `v5tail: bad magic — expected SFV51TAL, got ${Array.from(
          bytes.subarray(0, 8),
  ...
  if (version !== VERSION_V1 && version !== VERSION) {
    throw new Error(
      `v5tail: unsupported version ${version} (expected ${VERSION_V1} or ${VERSION})`,
    );
  }
  const variant = dv.getUint8(10);
  if (variant !== VARIANT_PER_CELL_AFFINE) {
    throw new Error(
      `v5tail: unsupported variant ${variant} (only per-cell affine = 2 implemented)`,
    );
  }
  ...
  if (mortonBytes.length !== kSelected * 4) {
    throw new Error(
      `v5tail: morton_idx size ${mortonBytes.length} != K*4 = ${kSelected * 4}`,
    );
  }
  const mortonIdx = new Uint32Array(
    mortonBytes.buffer.slice(mortonBytes.byteOffset, mortonBytes.byteOffset + mortonBytes.byteLength),
  );
  ...
  const cellOffsets = new Uint32Array(
    cellBytes.buffer.slice(cellBytes.byteOffset, cellBytes.byteOffset + cellBytes.byteLength),
  );
  ...
  const bitUnpack = (buf: Uint8Array, nValues: number, bd: number): Uint32Array => {
    ...
    if (bd === 8) {
      ...
      for (let i = 0; i < nValues; i++) out[i] = buf[i];
      return out;
    }
    if (bd === 16) {
    ...
    if (buf.length < needed) {
      throw new Error(
        `v5tail: bit_unpack ${bd}-bit truncated: have ${buf.length} need ${needed}`,
      );
    }
  ...
  const groupNChan = (gi: number): number => {
    switch (gi) {
      case 0: return 3;             // pos
      case 1: return 4;             // rot
      case 2: return 1;             // opa
  ...
  const dequantGroup = (gi: number): Float32Array => {
    ...
    if (metaRaw.length !== metaFloats * 4) {
      throw new Error(
        `v5tail: group ${gi} meta size ${metaRaw.length} != expected ${metaFloats * 4}`,
      );
    }
    const meta = new Float32Array(
      metaRaw.buffer.slice(metaRaw.byteOffset, metaRaw.byteOffset + metaRaw.byteLength),
    );
 ...
 */
export function applyV5TailToScene(
  scene: ApplyTargetScene,
  decoded: DecodedV5Tail,
): number {
  const k = decoded.header.kSelected;
  const n = scene.positions.length / 3;
  if (decoded.header.nSplats !== n) {
    throw new Error(
      `v5tail apply: sidecar n_splats ${decoded.header.nSplats} != scene count ${n}`,
    );
  }
