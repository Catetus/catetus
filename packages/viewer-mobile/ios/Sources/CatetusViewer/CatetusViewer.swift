// CatetusViewer — SwiftUI entry point.
//
...
public struct CatetusViewer: View {
    ...
    public var body: some View {
        CatetusMetalView(assetURL: assetURL)
    }
}
...
struct CatetusMetalView: PlatformMetalContainer {
    let assetURL: URL

    func makeCoordinator() -> CatetusRenderer {
        CatetusRenderer()
    }

    private func buildMTKView(coordinator: CatetusRenderer) -> MTKView {
        let view = MTKView()
    ...
    func makeUIView(context: Context) -> MTKView {
        ...
        let pan = UIPanGestureRecognizer(target: context.coordinator,
                                         action: #selector(CatetusRenderer.handlePan(_:)))
        let pinch = UIPinchGestureRecognizer(target: context.coordinator,
                                             action: #selector(CatetusRenderer.handlePinch(_:)))
    ...
    func makeNSView(context: Context) -> MTKView {
        ...
        let pan = NSPanGestureRecognizer(target: context.coordinator,
                                         action: #selector(CatetusRenderer.handleMacPan(_:)))
        let mag = NSMagnificationGestureRecognizer(target: context.coordinator,
                                                   action: #selector(CatetusRenderer.handleMacMagnify(_:)))
        v.addGestureRecognizer(pan)
