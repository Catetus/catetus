// CatetusRenderer — point-sprite renderer with orbit camera + Rust depth sort.
//
// Pipeline per frame:
// 1. (one-time at load) `ctmv_decode_glb` → vertex buffer of `SplatVertex`.
...
// 3. Call `ctmv_sort_by_depth` to produce a fresh back-to-front index buffer.
...
// `CatetusShaders` (RadixSort.metal, SplatDecode.metal, ProjectGather.metal,
...
import Foundation
import MetalKit
import CatetusViewerC
import CatetusShaders
import simd
...
public final class CatetusRenderer: NSObject, MTKViewDelegate {
    ...
    private func loadInBackground(url: URL) {
        ...
        let status = data.withUnsafeBytes { (raw: UnsafeRawBufferPointer) -> SfmvStatus in
            ...
            return ctmv_decode_glb(base, raw.count, &handle)
        }
        guard status == SfmvStatusOk, let h = handle else { return }
        let count = ctmv_buffer_len(h)
        let stride = ctmv_vertex_stride()
        guard let src = ctmv_buffer_data(h) else { ctmv_buffer_free(h); return }
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self = self, let device = self.device else { ctmv_buffer_free(h); return }
        ...
        if let vb = vertexBuffer, let ib = indexBuffer, splatCount > 0, let h = bufferHandle {
            ...
            indexScratch.withUnsafeMutableBufferPointer { idxPtr in
                view.withUnsafeBufferPointer { vp in
                    _ = ctmv_sort_by_depth(h, vp.baseAddress, idxPtr.baseAddress)
                }
            }
    ...
    private func makePipeline(device: MTLDevice, pixelFormat: MTLPixelFormat) -> MTLRenderPipelineState? {
        ...
        // `CatetusShaders` target; that's where the compiled .metallib
        // lives at runtime.
        let library: MTLLibrary?
        do {
            library = try device.makeDefaultLibrary(bundle: CatetusShaders.bundle)
        } catch {
            library = nil
        }
