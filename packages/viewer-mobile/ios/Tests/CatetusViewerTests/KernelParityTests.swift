...
// `Sources/CatetusViewer/Shaders/`. Each test compiles the relevant .metal
...
final class KernelParityTests: XCTestCase {
    ...
    /// .metal files are copied into the CatetusViewer resource bundle by
    ...
    private static func compileLibrary(named name: String, device: MTLDevice) throws -> MTLLibrary {
        // The .metal sources live in the `CatetusShaders` resource bundle.
        guard let src = CatetusShaders.source(forKernel: name) else {
            throw NSError(domain: "ParityTests", code: 2,
                          userInfo: [NSLocalizedDescriptionKey: "missing \(name).metal in CatetusShaders bundle"])
        }
