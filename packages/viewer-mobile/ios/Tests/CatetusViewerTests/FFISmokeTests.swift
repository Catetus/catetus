...
// `CatetusViewerCore.xcframework` binary target. This is the first test in
...
// outside the repo). Instead we use the workspace's `catetus-gltf` writer
...
final class FFISmokeTests: XCTestCase {
    ...
    /// `ctmv_vertex_stride()` returns the 56-byte SplatVertex stride. This is
    ...
    func testVertexStrideMatches() {
        XCTAssertEqual(ctmv_vertex_stride(), 56,
    ...
    func testDecodeNullReturnsNullPointerStatus() {
        ...
        let status = ctmv_decode_glb(nil, 0, &handle)
    ...
    func testDecodeGarbageReturnsDecodeFailed() {
        ...
        let status = bytes.withUnsafeBufferPointer { buf -> SfmvStatus in
            return ctmv_decode_glb(buf.baseAddress, buf.count, &handle)
        }
        XCTAssertEqual(status, SfmvStatusDecodeFailed)
        XCTAssertNil(handle)
    }

    /// `ctmv_buffer_len` and `ctmv_buffer_data` must tolerate a NULL handle
    /// (return 0 / NULL respectively, never crash).
    func testBufferAccessorsTolerateNull() {
        XCTAssertEqual(ctmv_buffer_len(nil), 0)
        XCTAssertNil(ctmv_buffer_data(nil))
    }
}
