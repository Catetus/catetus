...
Native SwiftUI splat viewer. Renders SplatForge `.glb` / `.spz` Gaussian-splat
...
| Target              | Role                                                          |
|---------------------|---------------------------------------------------------------|
| `SplatforgeShaders` | Resource-only Swift target hosting the MSL compute kernels.   |
| `SplatforgeViewerC` | Cbindgen-generated C ABI shim onto the Rust `splatforge-viewer-mobile` staticlib. |
| `SplatforgeViewer`  | SwiftUI view + `MTKView` renderer.                            |
| `SplatforgeViewerTests` | Kernel-parity tests for the MSL compute kernels.          |

## Shaders

Compute kernels live in `Sources/SplatforgeShaders/Shaders/`:
...
for f in SplatDecode RadixSort HistogramSubgroup ScanMultiblock ProjectGather; do
  xcrun -sdk iphoneos metal -c Sources/SplatforgeShaders/Shaders/$f.metal -o /tmp/$f.air
done
```
...
(`sfmv_decode_glb`, etc.) link only via the XCFramework. To produce that:

```
rustup target add aarch64-apple-ios aarch64-apple-ios-sim x86_64-apple-ios
../scripts/regen-headers.sh
../scripts/build-xcframework.sh
# then add SplatforgeViewerCore.xcframework as a binaryTarget in Package.swift
```
