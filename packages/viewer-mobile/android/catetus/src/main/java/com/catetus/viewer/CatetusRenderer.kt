// CatetusRenderer — GLES 3.1 point-sprite renderer.
//
...
internal class CatetusRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        GLES31.glVertexAttribPointer(0, 3, GLES31.GL_FLOAT, false, CatetusNative.vertexStride(), off); off += 12
        GLES31.glVertexAttribDivisor(0, 1)
        GLES31.glEnableVertexAttribArray(1)
        GLES31.glVertexAttribPointer(1, 4, GLES31.GL_FLOAT, false, CatetusNative.vertexStride(), off); off += 16
        GLES31.glVertexAttribDivisor(1, 1)
        GLES31.glEnableVertexAttribArray(2)
        GLES31.glVertexAttribPointer(2, 3, GLES31.GL_FLOAT, false, CatetusNative.vertexStride(), off); off += 12
        GLES31.glVertexAttribDivisor(2, 1)
        GLES31.glEnableVertexAttribArray(3)
        GLES31.glVertexAttribPointer(3, 1, GLES31.GL_FLOAT, false, CatetusNative.vertexStride(), off); off += 4
        GLES31.glVertexAttribDivisor(3, 1)
        GLES31.glEnableVertexAttribArray(4)
        GLES31.glVertexAttribPointer(4, 3, GLES31.GL_FLOAT, false, CatetusNative.vertexStride(), off)
    ...
    private fun loadAsset(uri: Uri) {
        ...
        val newHandle = CatetusNative.decodeGlb(bytes)
        if (newHandle == 0L) return
        if (bufferHandle != 0L) CatetusNative.freeBuffer(bufferHandle)
        bufferHandle = newHandle
        splatCount = CatetusNative.bufferLen(newHandle)
        val byteCount = splatCount * CatetusNative.vertexStride()
        val staging = ByteBuffer.allocateDirect(byteCount).order(ByteOrder.nativeOrder())
        CatetusNative.copyVertices(newHandle, staging)
        staging.rewind()
        val buf = IntArray(1)
        if (vbo == 0) {
