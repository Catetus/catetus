// SplatforgeNative — thin JNI binding to the Rust C ABI.
//
...
// `src/main/jniLibs/<abi>/libsplatforge_viewer_mobile.so`.
//
// We do NOT use the C ABI directly from JNI — instead we hand-write a tiny
// `jni/sfmv_jni.c` shim that converts `jbyteArray` → `(const uint8_t*, size_t)`
...
@Keep
internal object SplatforgeNative {
    init {
        System.loadLibrary("splatforge_viewer_mobile")
    }
