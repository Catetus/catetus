// SplatforgeViewer — Android entry point.
//
// Public API mirrors the iOS Swift SDK: instantiate the view, hand it a
// `Uri` pointing at a `.glb` (file:// or content://), and it renders.
//
// Internally we extend `GLSurfaceView` and drive an `SplatforgeRenderer`
// that calls into the Rust core through `SplatforgeNative`. The Vulkan
 ...
 * ```kotlin
 * val viewer = SplatforgeViewer(context).apply {
 *     setAsset(Uri.parse("file:///android_asset/bonsai-7k.glb"))
 * }
 * setContentView(viewer)
 * ```
 */
class SplatforgeViewer @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null
) : GLSurfaceView(context, attrs) {

    private val renderer = SplatforgeRenderer(context)
