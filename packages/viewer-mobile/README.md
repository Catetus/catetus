# Catetus Mobile Viewer SDK

Native iOS + Android SDKs for rendering Catetus `.glb` / `.spz` Gaussian-splat
assets without a WKWebView / Chrome WebView. Both wrappers are thin shells
around a shared Rust core (`catetus-viewer-mobile`) that ports the
...
├─ ios/                        # SwiftPM package: CatetusViewer
│  ├─ Package.swift
│  ├─ Sources/CatetusViewer/{CatetusViewer,CatetusRenderer}.swift
│  ├─ Sources/CatetusViewer/Shaders/{SplatPointSprite,RadixSort,ProjectCovariance}.metal
│  └─ Sources/CatetusViewerC/include/catetus_viewer_mobile.h
├─ android/catetus/         # Kotlin AAR module: com.catetus.viewer
│  ├─ build.gradle.kts
│  └─ src/main/{java,res/raw,jni,AndroidManifest.xml}
├─ examples/iOS-Demo/          # SwiftUI demo wiring CatetusViewer to bonsai-7k.glb
...
| Component                                              | Status                  |
|--------------------------------------------------------|-------------------------|
| Rust core (`cargo build -p catetus-viewer-mobile`)  | builds, clippy clean    |
| Round-trip tests (`cargo test -p catetus-viewer-mobile`) | 3/3 pass           |
...
| Android AAR build (`./gradlew :catetus:assemble`)   | PENDING — requires NDK  |
...
`ios/Sources/CatetusShaders/Shaders/`:
...
```
xcrun -sdk iphoneos metal -c packages/viewer-mobile/ios/Sources/CatetusShaders/Shaders/SplatDecode.metal -o /tmp/SplatDecode.air
xcrun -sdk iphoneos metal -c packages/viewer-mobile/ios/Sources/CatetusShaders/Shaders/RadixSort.metal -o /tmp/RadixSort.air
xcrun -sdk iphoneos metal -c packages/viewer-mobile/ios/Sources/CatetusShaders/Shaders/HistogramSubgroup.metal -o /tmp/HistogramSubgroup.air
xcrun -sdk iphoneos metal -c packages/viewer-mobile/ios/Sources/CatetusShaders/Shaders/ScanMultiblock.metal -o /tmp/ScanMultiblock.air
xcrun -sdk iphoneos metal -c packages/viewer-mobile/ios/Sources/CatetusShaders/Shaders/ProjectGather.metal -o /tmp/ProjectGather.air
...
(cd packages/viewer-mobile/android && ./gradlew :catetus:assemble)
```
