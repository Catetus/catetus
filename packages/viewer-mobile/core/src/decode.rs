...
//!
//! Thin shim over `catetus_gltf::read_glb_bytes`: that workspace crate
//! parses the glTF JSON + binary chunk; we flatten the resulting
//! [`catetus_core::SplatScene`] into the packed render layout. Doing the
...
use catetus_gltf::{read_glb_bytes, GltfError};
use thiserror::Error;
