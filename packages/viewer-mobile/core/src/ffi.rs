...
//!   handle and read pointer + len via [`ctmv_buffer_data`] / [`ctmv_buffer_len`].
//! * The caller MUST call [`ctmv_buffer_free`] exactly once per handle.
//!
...
/// with [`ctmv_buffer_free`]. On failure `*out` is set to null and the
...
/// `*mut *mut SfmvBuffer`.
#[no_mangle]
pub unsafe extern "C" fn ctmv_decode_glb(
    bytes: *const c_uchar,
    len: usize,
...
///
/// # Safety
/// `buf` must be a live handle returned by [`ctmv_decode_glb`].
#[no_mangle]
pub unsafe extern "C" fn ctmv_buffer_data(buf: *const SfmvBuffer) -> *const c_void {
    if buf.is_null() {
        return std::ptr::null();
    }
...
#[no_mangle]
pub unsafe extern "C" fn ctmv_buffer_len(buf: *const SfmvBuffer) -> usize {
    if buf.is_null() {
        return 0;
    }
    let b = &*(buf as *const OwnedBuffer);
...
#[no_mangle]
pub extern "C" fn ctmv_vertex_stride() -> usize {
    SplatVertex::STRIDE
}

/// Free a buffer returned by [`ctmv_decode_glb`].
///
/// # Safety
/// `buf` must have come from [`ctmv_decode_glb`] and not yet been freed.
#[no_mangle]
pub unsafe extern "C" fn ctmv_buffer_free(buf: *mut SfmvBuffer) {
    if buf.is_null() {
        return;
    }
...
/// `out_indices` (caller-allocated; must be at least `ctmv_buffer_len`
/// entries).
///
...
#[no_mangle]
pub unsafe extern "C" fn ctmv_sort_by_depth(
