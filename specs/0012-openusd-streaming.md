...
Define how Catetus delivers **chunked, LOD'd Gaussian Splat assets through native OpenUSD composition primitives** — payloads, references, and variant sets — without inventing a new container format and without requiring viewer support for any Catetus-specific extension.
...
(
    ...
    upAxis = "Y"
    customLayerData = {
        string "catetus:version" = "0.2"
        string "catetus:ordering" = "morton"
        int "catetus:chunkCount" = 64
    }
)
...
def Xform "World"
{
    def Scope "Splats"
    {
        ...
        )
        {
            ...
            custom float3 catetus:bboxMin = (-1.0, -1.0, -1.0)
            custom float3 catetus:bboxMax = ( 1.0,  1.0,  1.0)
            custom int catetus:splatCount = 12500
            custom int catetus:loadPriority = 0
            custom string catetus:checksum = "blake3:abcdef0123..."
        }
...
| `chunks[i].bbox`                                 | `catetus:bboxMin` / `catetus:bboxMax` custom attributes on the chunk prim. |
| `chunks[i].splatCount`                           | `catetus:splatCount` custom attribute. |
| `chunks[i].checksum`                             | `catetus:checksum` custom attribute. |
| `chunks[i].loadPriority`                         | `catetus:loadPriority` custom attribute. The viewer-side scheduler reads it; USD itself has no priority concept. |
| `chunks[i].lod`                                  | Variant selection on the `"lod"` variant set. |
| `ordering` ("morton")                            | `customLayerData["catetus:ordering"]` on the root layer. |
| `lods[]` array of `{ level, splatFraction }`     | Per-variant `catetus:splatFraction` custom attribute. |

The bounding box, splat count, checksum, and load priority are not part of the USD core schema, so they live under the `catetus:` custom-attribute namespace — mirroring the "namespace your additions" pattern from SPEC-0007. A renderer that ignores them gets correct visuals (it just can't prioritize loading).
...
* On stage open, USD will not load any chunk by default. The viewer (or Catetus's planned `catetus-viewer-usd`) is responsible for calling `UsdStage::Load()` on chunks in load-priority order.
...
There is no Catetus-specific extension required to render the asset; all `catetus:` custom attributes are optional hints.
...
## File layout (planned, in `catetus-usd`)
...
Scenario: Export chunked USD
  ...
  When I run "catetus optimize tiny/basic_binary.ply --chunked --to usd --out scene.usda"
...
Scenario: Each chunk has bounding metadata
  ...
  Then every chunk prim carries catetus:bboxMin and catetus:bboxMax
  And every chunk prim carries catetus:splatCount
  And every chunk prim carries catetus:checksum starting with "blake3:"
...
Scenario: Corrupted chunk is detected
  ...
  Then "catetus inspect scene.usda" reports a checksum mismatch for chunk_0000
...
Scenario: Renderer without catetus: namespace still renders
  Given a chunked USD output
  When a viewer that ignores catetus:* custom attributes loads the stage
...
* The `catetus:` custom-attribute namespace is the same namespace used in SPEC-0011 for fields with no native USD analog. No new namespaces are introduced.
...
7. **Viewer responsibility.** This spec describes the asset layout; it does not specify a USD-aware viewer. Catetus's current viewer SDK (SPEC-0008) is glTF-only. A `catetus-viewer-usd` is a separate effort and is not blocked by this spec.
