...
Shrink the glTF wire size of an optimized Catetus scene so the glTF-only
...
median; see `catetus-optimize::QuantizeScale`).
...
Feature: KHR_mesh_quantization on optimize output
  ...
  Scenario: web-mobile preset emits integer POSITION accessor
    ...
    When `catetus optimize` writes the glTF
  ...
  Scenario: writer + reader round-trip preserves per-axis quantization
    ...
    When the glTF is written and read back via `catetus convert`
...
1. **Extend `catetus_gltf::GltfAccessor`** with optional `normalized`,
...
2. **Add a `quantize` flag to `WriteOpts`** that defaults to `false`; preset
   presets in `catetus-optimize` flip it on for the three quantization-
   safe ones.
