...
> This spec is a design proposal for v0.2 work. It has **not** been validated against a real OpenUSD toolchain in this repo; there is currently no `splatforge-usd` crate, no `usdview` dependency, and no fixture USDA files. Every attribute name, type, and prim path below MUST be confirmed against an actual OpenUSD build (≥ v26.03) before implementation lands.
...
```
splatforge convert <in> --to usd --out scene.usda
splatforge convert scene.usda --to gltf --out scene.gltf
```
...
```
/World                                    # Xform (root)
    metadata: customLayerData["splatforge:version"] = "0.2"
    metadata: customLayerData["splatforge:coordinateSystem"] = "y_up_right_handed"
...
| per-splat ID         | `ids` (?)                                         | `int64[]`        | Optional. Used by editors for stable selection across re-exports. If schema lacks this, use `splatforge:ids` custom attribute. |
| semantic labels      | `splatforge:semanticLabels`                       | `int[]`          | No native USD analog. Custom attribute namespace. |
| coordinate system    | layer metadata `customLayerData`                  | dict             | `upAxis` is also written as a standard USD stage metadata. |
| temporal mode        | `splatforge:temporalMode`                         | `token`          | Always `"static"` in v1. |
...
* Stage metadata `upAxis` is written. USD supports `"Y"` or `"Z"`; SplatForge writes whatever the IR carries.
* USD does not encode handedness as stage metadata. SplatForge records it in `customLayerData["splatforge:coordinateSystem"]`. On import, if the value is absent, the importer assumes right-handed (USD convention).
...
Scenario: Fields with no USD analog land in splatforge namespace
  Given an IR with semantic labels
  When I export to USDA
  Then the prim carries attribute "splatforge:semanticLabels"
...
Scenario: Import USDA produced by another tool
  ...
  When I run "splatforge analyze scene.usda"
...
* No existing SplatForge artifact format changes. USD is purely additive.
