#!/usr/bin/env bash
# Catetus first-time setup. Idempotent — safe to re-run.
#
# Usage:
...
# Build
# ----------------------------------------------------------------------------
say "Building catetus CLI..."
if [ "$QUICK" -eq 1 ]; then
  cargo check --workspace
  ok "cargo check passed"
else
  cargo build --release -p catetus-cli
  ok "built target/release/catetus"
fi
...
if [ "$QUICK" -eq 0 ]; then
  ...
  ./target/release/catetus analyze fixtures/tiny/basic_binary.ply --pretty > /tmp/_sf_smoke.json
  if grep -q '"splatCount": 3' /tmp/_sf_smoke.json; then
    ok "catetus analyze works"
  else
...
${GREEN}Catetus is ready.${RESET}

Try:
  ./target/release/catetus analyze fixtures/tiny/basic_binary.ply --pretty
  ./target/release/catetus optimize fixtures/tiny/basic_binary.ply --preset web-mobile --out /tmp/scene.gltf
  ./target/release/catetus inspect /tmp/scene.gltf
  make help
