...
- [ ] Public repo `catetus/catetus` exists.
- [ ] Public tap repo exists: `gh repo create catetus/homebrew-tap --public`.
      Seed it with a `README.md` and a `Formula/` directory.
- [ ] npm scope owned: `npm org create catetus` and `npm login` as a
      member of the org.
...
- [ ] Bump `distrib/npm/package.json#version` to **match exactly** — the
      postinstall constructs the URL as `v${pkg.version}/catetus-v${pkg.version}-…`,
...
1. **Tag and push.**
    ...
    then publishes a GitHub Release with:
      ...
      - `catetus-vX.Y.Z-<target>.tar.gz` × 4 (unix targets)
      - `catetus-vX.Y.Z-x86_64-pc-windows-msvc.zip`
      - `*.sha256` siblings
...
3. **Verify:**
    ...
    ```bash
    gh release view vX.Y.Z --repo catetus/catetus
    ```
...
git diff distrib/homebrew/catetus.rb
git add distrib/homebrew/catetus.rb
...
```bash
cp distrib/homebrew/catetus.rb \
   /path/to/homebrew-tap/Formula/catetus.rb
( cd /path/to/homebrew-tap && \
  git add Formula/catetus.rb && \
  git commit -m "catetus vX.Y.Z" && \
  git push origin main )
```
...
```bash
npm install -g @catetus/cli
catetus --version
# Or, without polluting the global cache:
npx --yes @catetus/cli@latest --version
```
...
2. **npm** — within 72 h: `npm unpublish @catetus/cli@X.Y.Z`. After
   72 h: ship X.Y.Z+1 and `npm deprecate @catetus/cli@X.Y.Z "use X.Y.Z+1"`.
...
The eventual goal is to make all three publishes automatic from a tag push:
  ...
  - Step 2 (Homebrew): a follow-up job on `release.yml` that runs
    ...
    repo via `gh pr create --repo catetus/homebrew-tap`. Needs a PAT
