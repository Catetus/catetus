...
[Catetus](https://github.com/catetus/catetus) CLI, fetched as a
native Rust binary on `npm install`.

```bash
npm install -g @catetus/cli
catetus --help
catetus-khr-validate path/to/scene.gltf
catetus-usd-validate path/to/scene.usdc
```

## What this package does

This is a **thin npm wrapper** around the native Rust binaries published
to [Catetus's GitHub releases](https://github.com/catetus/catetus/releases).
On `npm install` the `postinstall` step:

1. Detects your OS + architecture.
2. Downloads the matching pre-built archive
   (e.g. `catetus-v0.1.0-aarch64-apple-darwin.tar.gz`).
...
The three published CLIs (`catetus`, `catetus-khr-validate`,
`catetus-usd-validate`) are exposed as standard npm `bin` entries, so
...
```bash
cargo install --git https://github.com/catetus/catetus \
  --locked catetus-cli
```

## Offline / no-network installs

Set `CATETUS_SKIP_DOWNLOAD=1` to bypass the postinstall fetch. You'll
need to drop the native binary into the package's `native/` directory
yourself.

```bash
CATETUS_SKIP_DOWNLOAD=1 npm install -g @catetus/cli
# Then manually:
curl -L https://github.com/catetus/catetus/releases/download/v0.1.0/<archive> \
  | tar -xz -C $(npm root -g)/@catetus/cli/native
```
