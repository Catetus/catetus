#!/usr/bin/env node
// Catetus npm postinstall.
//
// Downloads the per-arch binary archive for the current package version
// from the Catetus GitHub release, verifies its SHA-256 against the
// release's SHASUMS256.txt manifest, and extracts the three binaries
// (`catetus`, `catetus-khr-validate`, `catetus-usd-validate`)
...
//                              `npm i -g @catetus/cli` would "succeed"
//                              with a broken `catetus` shim and the
...
//   - CATETUS_SKIP_DOWNLOAD=1 -> bypasses the download (test/CI).
...
const BINS = ["catetus", "catetus-khr-validate", "catetus-usd-validate"];

function fail(msg) {
  // Use stderr + non-zero exit. Don't `throw` — npm prints throws as
  // unhelpful stack traces to the user.
  process.stderr.write(`\n[@catetus/cli postinstall] ${msg}\n\n`);
  process.exit(1);
}

function info(msg) {
  process.stdout.write(`[@catetus/cli] ${msg}\n`);
}
...
function targetForHost() {
  ...
  if (!target) {
    fail(
      ...
      `Install from source via 'cargo install --path crates/catetus-cli' instead.`
    );
  }
  return target;
}
...
function download(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, { headers: { "User-Agent": "catetus-npm-installer" } }, (res) => {
...
function fetchText(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { "User-Agent": "catetus-npm-installer" } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
...
// --------------------------------------------------------------------------
(async () => {
  if (process.env.CATETUS_SKIP_DOWNLOAD === "1") {
    info("CATETUS_SKIP_DOWNLOAD=1 — skipping binary download.");
    return;
  }
  ...
  const archiveName = `catetus-${TAG}-${target}.${ext}`;
  ...
  try {
    ...
    fail(
      ...
      `CATETUS_SKIP_DOWNLOAD=1 and install the binary manually from ` +
      `${RELEASE_URL}.`
    );
  }
