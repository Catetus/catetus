#!/usr/bin/env bash
# Update distrib/homebrew/catetus.rb with the version + SHA-256 hashes for
...
#      distrib/homebrew/catetus.rb in-place.
...
TAG="$1"
VERSION="${TAG#v}"
REPO="catetus/catetus"
FORMULA_PATH="$(cd "$(dirname "$0")/../.." && pwd)/distrib/homebrew/catetus.rb"
...
declare -A targets=(
  [arm_mac]="catetus-${TAG}-aarch64-apple-darwin.tar.gz"
  [x86_mac]="catetus-${TAG}-x86_64-apple-darwin.tar.gz"
  [x86_lin]="catetus-${TAG}-x86_64-unknown-linux-gnu.tar.gz"
  [arm_lin]="catetus-${TAG}-aarch64-unknown-linux-gnu.tar.gz"
)
...
echo "  1. Review the diff: git diff distrib/homebrew/catetus.rb"
echo "  2. Commit it to the main repo."
echo "  3. Copy the file into catetus/homebrew-tap and push."
