...
#   - SplatForge cloned on the 4090 box at $REMOTE_REPO (default
#     ~/SplatForge). The branch with the perf changes must be checked out
...
REMOTE_REPO="${REMOTE_REPO:-\$HOME/SplatForge}"
BRANCH="${BRANCH:-}"
...
REMOTE_CMD="$REMOTE_CMD && pnpm --filter @splatforge/viewer run bench"
