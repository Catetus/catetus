...
straight from Blender. Wraps the production `catetus` CLI and the
Catetus Cloud API in a clean N-panel.
...
* `catetus` CLI installed somewhere on disk. The add-on auto-detects it
  on PATH and at well-known install dirs (homebrew, `~/.cargo/bin`,
  `~/.local/bin`, `%LOCALAPPDATA%\Programs\catetus`, `C:\ProgramData\chocolatey\bin`).
  ...
  cargo install catetus-cli

  # Windows — grab catetus.exe from
  # https://github.com/Catetus/catetus/releases and put it on PATH
  ```

## Install

1. Download `catetus_addon-<version>.zip` from
   [GitHub Releases](https://github.com/Catetus/catetus/releases)
   (or build it locally — see *Build the zip* below).
2. In Blender: **Edit ▸ Preferences ▸ Add-ons ▸ Install…** and pick the .zip.
3. Tick the **Catetus** entry to enable it.
4. Expand the add-on row to set your **API key** (optional — only needed for
   *Submit to Cloud*) and confirm the **catetus CLI** field shows a
...
1. **Import Splat** — file-select your `.ply` / `.spz` / `.glb`. Non-glb
   files are auto-converted via `catetus convert` so Blender's stock
...
3. **Optimize** — runs `catetus optimize` and streams progress into the
...
4. **Submit to Cloud** — same preset, but on managed compute. Uses your
   `CATETUS_API_KEY` (from the add-on prefs, env var, or
   `~/.config/catetus/api_key`). The poller runs on a worker thread
...
# → writes integrations/blender/dist/catetus_addon-<version>.zip
```
...
* zips it with the `catetus_addon/` directory at the zip root (the
...
| Symptom | Fix |
|---|---|
| `catetus CLI not found` red banner | Click the magnifier icon to re-probe, or paste an absolute path in *Add-on Preferences ▸ catetus CLI*. If you installed via cargo, the path is usually `~/.cargo/bin/catetus`. |
| `catetus CLI not found` on Windows | Download `catetus.exe` from Releases, put it next to your project or somewhere on `PATH`, then re-probe. |
...
`catetus_source` / `catetus_glb` custom properties set to the
bonsai sample bundled with the SplatBench corpus. Open it, switch to the
*Catetus* sidebar tab, and click **Optimize** — you get a feel for
