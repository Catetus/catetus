"""SplatForge Blender add-on.

Wraps the ``splatforge`` CLI and SplatForge Cloud API so Blender users can
import a Gaussian-Splat file (.ply / .spz / .glb), optimize it for a target
device preset, and either keep the output locally or submit it to SplatForge
Cloud — all from an N-panel inside the 3D viewport.

The add-on does NOT bundle the CLI. It auto-detects the user's installed
``splatforge`` binary (homebrew / cargo-install / explicit path) and surfaces
...
bl_info = {
    ...
    "author": "SplatForge contributors",
    "version": (0, 1, 0),
    "blender": (4, 2, 0),
    "location": "View3D > Sidebar > SplatForge",
    "warning": "",
    "doc_url": "https://github.com/montabano1/SplatForge/tree/main/integrations/blender",
    "tracker_url": "https://github.com/montabano1/SplatForge/issues",
    "support": "COMMUNITY",
