"""Locate the user's ``splatforge`` CLI binary.
...
suspects, in priority order:
  ...
  2. ``$SPLATFORGE_CLI`` env var.
  3. ``shutil.which("splatforge")`` — covers any binary on ``$PATH`` already.
     This is what 95% of users (homebrew, cargo-install, manual /usr/local
     install) will hit.
  4. Platform-specific well-known install dirs:
       - macOS:  ``/opt/homebrew/bin/splatforge`` (Apple Silicon brew)
                 ``/usr/local/bin/splatforge``    (Intel brew, manual)
                 ``~/.cargo/bin/splatforge``      (cargo install)
       - Linux:  ``/usr/local/bin/splatforge``
                 ``~/.cargo/bin/splatforge``
                 ``~/.local/bin/splatforge``
       - Windows: ``%USERPROFILE%\\.cargo\\bin\\splatforge.exe``
                  ``%LOCALAPPDATA%\\Programs\\splatforge\\splatforge.exe``
                  ``C:\\ProgramData\\chocolatey\\bin\\splatforge.exe``
...
CLI_NAME = "splatforge.exe" if sys.platform.startswith("win") else "splatforge"
...
class CliInfo:
    ...
    path: str
    version: str  # e.g. "splatforge 0.1.2" — the first line of `--version`
...
def _platform_search_paths() -> list[Path]:
    ...
    if sys.platform.startswith("win"):
        ...
        if localappdata:
            candidates.append(Path(localappdata) / "Programs" / "splatforge" / CLI_NAME)
...
def detect_cli(pref_path: Optional[str] = None) -> Optional[CliInfo]:
    ...
    env_path = os.environ.get("SPLATFORGE_CLI")
    info = _probe(env_path, "env:SPLATFORGE_CLI")
    if info is not None:
        return info
...
def install_hint() -> str:
    ...
    if sys.platform == "darwin":
        return "brew install splatforge   (or: cargo install splatforge-cli)"
    if sys.platform.startswith("linux"):
        return "cargo install splatforge-cli   (or: download the release tarball)"
    if sys.platform.startswith("win"):
        return (
            "Download splatforge.exe from "
            "https://github.com/montabano1/SplatForge/releases and add it to PATH"
        )
    return "cargo install splatforge-cli"
