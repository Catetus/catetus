...
2. **API key** — the single biggest UX risk per the brief. Most Blender
   ...
   ``$SPLATFORGE_API_KEY``. We surface a password-masked text field in
   ...
   POSIX), and additionally write it to ``~/.config/splatforge/api_key``
...
``SPLATFORGE_API_KEY`` in the child process environment regardless of
...
# (which Blender also uses), and ``%APPDATA%/SplatForge`` on Windows.
def _config_dir() -> Path:
    if os.name == "nt":
        base = Path(os.environ.get("APPDATA", str(Path.home() / "AppData/Roaming")))
        return base / "SplatForge"
    xdg = os.environ.get("XDG_CONFIG_HOME")
    if xdg:
        return Path(xdg) / "splatforge"
    return Path.home() / ".config" / "splatforge"
...
def _save_api_key_to_disk(key: str) -> None:
    """Persist the API key to ``~/.config/splatforge/api_key`` (chmod 600).
...
def api_key_for_env(prefs: "SplatForgePreferences") -> str:
    ...
    env = os.environ.get("SPLATFORGE_API_KEY")
    if env:
        return env
    return _load_api_key_from_disk()


def _on_api_key_update(self: "SplatForgePreferences", _context: bpy.types.Context) -> None:
...
class SPLATFORGE_OT_detect_cli(Operator):
    ...
    bl_idname = "splatforge.detect_cli"
    bl_label = "Detect splatforge CLI"
    bl_description = (
        "Search PATH and well-known install locations for the splatforge "
    ...
    def execute(self, context: bpy.types.Context):  # noqa: D401
        ...
        if info is None:
            self.report(
                {"ERROR"},
                "splatforge CLI not found. " + installer.install_hint(),
            )
            return {"CANCELLED"}
...
class SplatForgePreferences(AddonPreferences):
    bl_idname = __package__

    cli_path: StringProperty(
        name="splatforge CLI",
        description=(
            "Absolute path to the splatforge binary. Leave empty for auto-detect "
            "(PATH + well-known locations). Click 'Detect splatforge CLI' to refresh."
        ),
        subtype="FILE_PATH",
        default="",
    )
    ...
    api_url: StringProperty(
        name="API URL",
        description="SplatForge Cloud endpoint",
    ...
    api_key: StringProperty(
        name="API Key",
        description=(
            "Personal SplatForge Cloud API key. Mirrored to "
            "~/.config/splatforge/api_key (chmod 600) so the CLI can find it"
        ),
    ...
    def draw(self, _context: bpy.types.Context) -> None:
        ...
        box = layout.box()
        box.label(text="SplatForge CLI", icon="CONSOLE")
        row = box.row(align=True)
        row.prop(self, "cli_path", text="")
        row.operator(SPLATFORGE_OT_detect_cli.bl_idname, icon="VIEWZOOM", text="")
        if self.cli_version:
...
_CLASSES = (SPLATFORGE_OT_detect_cli, SplatForgePreferences)


def get_prefs(context: Optional[bpy.types.Context] = None) -> SplatForgePreferences:
