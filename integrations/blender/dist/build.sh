#!/usr/bin/env bash
# Package the Catetus add-on into a Blender-installable .zip.
#
...
#       (`catetus_addon/`) containing an `__init__.py` with `bl_info`.
...
#
# Usage:
#   ./integrations/blender/dist/build.sh            # writes ./dist/catetus_addon-<version>.zip
...
SRC_DIR="$ADDON_DIR/catetus_addon"

if [[ ! -d "$SRC_DIR" ]]; then
    echo "ERROR: $SRC_DIR not found — run from a Catetus checkout" >&2
    exit 2
fi
...
OUT="${OUT:-$SCRIPT_DIR/catetus_addon-${VERSION}.zip}"
mkdir -p "$(dirname "$OUT")"

# Stage in a temp dir to avoid shipping accidental files (.DS_Store, __pycache__).
STAGE="$(mktemp -d -t catetus-addon.XXXXXX)"
...
if [[ -f "$ADDON_DIR/README.md" ]]; then
    cp "$ADDON_DIR/README.md" "$STAGE/catetus_addon/README.md"
fi
...
(
    ...
    if command -v zip >/dev/null 2>&1; then
        zip -r -q "$OUT" catetus_addon
    else
        python3 -m zipfile -c "$OUT" catetus_addon
    fi
)
...
names = z.namelist()
assert any(n.startswith("catetus_addon/__init__.py") for n in names), \
    f"zip missing catetus_addon/__init__.py — got: {names[:5]}"
