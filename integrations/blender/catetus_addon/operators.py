"""Blender operators that drive the Catetus CLI + Cloud API.
...
This keeps the UI responsive — submitting a 200 MB scene to Catetus
Cloud no longer locks the viewport.

Five operators are exposed:

* ``catetus.import_splat``       — file-browser import (.ply/.spz/.glb)
* ``catetus.optimize_local``     — run ``catetus optimize``
* ``catetus.submit_to_cloud``    — POST /v1/jobs + upload + poll
* ``catetus.fetch_fidelity``     — run ``catetus fidelity``
* ``catetus.open_share_link``    — open the last cloud URL in a browser
"""
...
def _ensure_cli(operator: Operator) -> Optional[str]:
    """Resolve the catetus CLI path or report an actionable error.
    ...
    if info is None:
        operator.report(
            {"ERROR"},
            f"catetus CLI not found. {installer.install_hint()}",
        )
        return None
...
def _child_env(prefs) -> dict[str, str]:
    ...
    if api_key:
        env["CATETUS_API_KEY"] = api_key
    if prefs.api_url:
        env["CATETUS_API_URL"] = prefs.api_url
    return env
...
class CATETUS_OT_import_splat(Operator):
    ...
    ``catetus convert`` and then imported the same way — this means the
    ...
    bl_idname = "catetus.import_splat"
    ...
    def execute(self, _context):
        ...
        for obj in bpy.context.selected_objects:
            obj["catetus_source"] = str(src)
            obj["catetus_glb"] = str(glb_path)
...
def _selected_source_path() -> Optional[Path]:
    ...
    for obj in [bpy.context.active_object, *bpy.context.selected_objects]:
        if obj is None:
            continue
        src = obj.get("catetus_source") or obj.get("catetus_glb")
        if src:
            return Path(bpy.path.abspath(str(src)))
    return None


class CATETUS_OT_optimize_local(Operator):
    """Run ``catetus optimize`` against the selected splat file.
    ...
    bl_idname = "catetus.optimize_local"
    bl_label = "Optimize"
    bl_description = (
        "Run the catetus CLI with the chosen preset on the imported splat. "
...
class CATETUS_OT_submit_to_cloud(Operator):
    """Submit the selected splat to Catetus Cloud.
    ...
    bl_idname = "catetus.submit_to_cloud"
    bl_label = "Submit to Cloud"
    bl_description = (
        "Upload the selected splat to Catetus Cloud, run the chosen preset "
    ...
    def execute(self, context):
        ...
        self._worker = threading.Thread(
            ...
            args=(
                ...
                self.label,
                prefs.api_url,
                prefs.api_key or os.environ.get("CATETUS_API_KEY", ""),
                self._q,
                self._stop_flag,
            ),
            daemon=True,
    ...
    def _run_submit(
        ...
        api = api_url.rstrip("/") if api_url else "https://catetus-api.fly.dev"
...
class CATETUS_OT_open_share_link(Operator):
    """Open the last successful cloud job's share URL in the system browser."""

    bl_idname = "catetus.open_share_link"
    bl_label = "Open Share Link"
    bl_description = "Open the Catetus Cloud job page in your browser"
...
_CLASSES = (
    CATETUS_OT_import_splat,
    CATETUS_OT_optimize_local,
    CATETUS_OT_submit_to_cloud,
    CATETUS_OT_fetch_fidelity,
    CATETUS_OT_open_share_link,
)
