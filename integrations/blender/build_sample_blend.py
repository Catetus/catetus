...
labelled "Bonsai Splat" with the ``splatforge_source`` /
``splatforge_glb`` custom properties pre-filled, plus a camera positioned
so the SplatForge N-panel is visible on open. The Empty's stamped paths
...
def make_empty() -> None:
    ...
    obj["splatforge_source"] = DEMO_SOURCE
    obj["splatforge_glb"] = DEMO_SOURCE  # same file — the import op will convert lazily
...
def write_sample() -> None:
    ...
    # Default preset stamp on the scene — the panel reads ``scene.splatforge``
    # if the add-on is enabled, and falls back gracefully if it isn't.
    bpy.context.scene["splatforge_default_preset"] = "web-mobile"
