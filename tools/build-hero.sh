#!/usr/bin/env bash
# build-hero.sh — reusable rebuild pipeline for the SplatForge homepage hero
# asset. Wraps tools/build-hero.py (preprocess) and `splatforge optimize`
...
BIN="$REPO_ROOT/target/release/splatforge"
if [[ ! -x "$BIN" ]]; then
  err "splatforge release binary not found at $BIN"
  err "build it with: cargo build --release -p splatforge-cli"
  exit 3
fi
...
TMP_DIR="$(mktemp -d -t splatforge-hero.XXXXXX)"
...
info "step 2/4: splatforge optimize --preset web-mobile --chunked"
...
# splatforge optimize writes the splat count to the sidecar scene.json
