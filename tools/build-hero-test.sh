...
if [[ ! -x "$BIN" ]]; then
  echo "build-hero-test: splatforge release binary missing at $BIN" >&2
  echo "build-hero-test: run 'cargo build --release -p splatforge-cli' first" >&2
  exit 3
fi

TMP_DIR="$(mktemp -d -t splatforge-hero-test.XXXXXX)"
