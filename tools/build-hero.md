...
above the fold on **catetus.com**. It loads as a chunked glTF
...
`catetus optimize` pipeline so the output stays small enough to ship,
...
Two things go wrong if we feed that directly to `catetus optimize`:
...
The result is a clean ~150k-splat PLY that `catetus optimize` can
...
3. Runs `catetus optimize --preset web-mobile --chunked
   --out <out>/scene.gltf`.
...
2. **Serve locally:**
   ```bash
   pnpm --filter @catetus/web preview
   ```
...
```bash
cargo build --release -p catetus-cli  # one time
