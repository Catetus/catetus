...
# first Catetus commit. Idempotent and safe to re-run while WIP.
#
...
if ! git config user.email >/dev/null 2>&1; then
  git config user.email "monte@catetus.com"
  ok "set local user.email = monte@catetus.com"
fi
...
# ---------------------------------------------------------------------------
if [ -f bin/catetus ]; then
  rm -f bin/catetus
  ok "removed stale linux binary at bin/catetus"
fi
...
if [ -z "$STAGED" ]; then
  ...
  echo "    git remote add origin https://github.com/Catetus/catetus.git"
...
# ---------------------------------------------------------------------------
COMMIT_MSG=$(cat <<'EOF'
feat: initial Catetus v0.1.0 — Phase 0–2 + SplatBench v0
...
What's in this commit:
  ...
  * Rust workspace (7 crates):
      catetus-core      — canonical SplatIR + deterministic JSON report
      catetus-ply       — Inria 3DGS binary-LE + ASCII reader/writer
      catetus-spz       — v2 SPZ codec, round-trip parity tests
      catetus-gltf      — glTF 2.0 + KHR_gaussian_splatting + GLB writer,
                             SF_spatial_streaming_index vendor ext
      catetus-optimize  — 10-pass framework with 8 named presets
      catetus-bench     — corpus runner
      catetus-cli       — `catetus` binary, 8 subcommands

  * TypeScript packages:
      @catetus/viewer    — WebGPU + WebGL2 instanced-quad Gaussian
                              renderer with EWA fragment shading and
                              progressive chunked loading
      @catetus/report-ui — visual-diff + parity HTML templates

  * Visual harness (tests/visual) — Playwright 4-renderer parity matrix
    and end-to-end `catetus diff` Node helper.
...
Signed-off-by: Monte <monte@catetus.com>
EOF
)
...
if git remote get-url origin >/dev/null 2>&1; then
  ...
  say "adding remote: git remote add origin https://github.com/Catetus/catetus.git"
  git remote add origin https://github.com/Catetus/catetus.git
  ok "remote added"
fi
