...
**`SPLATFORGE_CAPTURE_URL` missing from worker Modal Secret `required_keys`.**
...
endpoint (SPLATFORGE_CAPTURE_URL) but none is configured" — even though
...
Also added test coverage for `splatforge-qat-bundle`, which had zero
...
band tests for some presets but not all (e.g. `splatforge-qat-bundle`,
...
in `PRESET_DISPATCH_URLS` or in the CLI's `splatforge optimize` known
...
Final green status:
- `cargo test -p splatforge-api`: 73 lib + 5 customer_dashboard + 8 audit + 12 billing + 12 license + 17 pricing + 13 ratelimit + 3 import + 18 ratings + 12 store_trait = 173 tests, all green.
