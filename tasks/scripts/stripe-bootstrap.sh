...
# that Catetus Cloud's paid tier expects.
#
...
RUNS_SKU="catetus_repack_runs"
SECS_SKU="catetus_repack_seconds"
...
# unlikely to have more than two Catetus meters in practice.
...
echo "==> Provisioning Catetus billing meters + products (test mode)" >&2

RUNS_METER_ID="$(ensure_meter "$RUNS_SKU" "Catetus repack runs")"
SECS_METER_ID="$(ensure_meter "$SECS_SKU" "Catetus repack compute seconds")"
...
RUNS_PRODUCT_ID="$(ensure_product \
  "catetus_repack_runs_product" \
  "Catetus Cloud — Repack Runs" \
...
SECS_PRODUCT_ID="$(ensure_product \
  "catetus_repack_seconds_product" \
  "Catetus Cloud — Repack Compute Seconds" \
...
echo "         stripe listen --forward-to https://catetus-api.fly.dev/v1/stripe/webhook" >&2
echo "       and copy the printed whsec_... below." >&2
echo "    3. Map each beta customer to their bearer key in" >&2
echo "       CATETUS_KEY_CUSTOMERS (see apps/api/BILLING.md)." >&2
echo "" >&2
...
CATETUS_KEY_CUSTOMERS=key1:cus_REPLACE_ME

# Reference (informational only — billing.rs uses the SKU constants):
#   catetus_repack_runs    meter -> $RUNS_METER_ID
#   catetus_repack_seconds meter -> $SECS_METER_ID
EOF
