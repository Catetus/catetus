...
(tasks/scripts/p2-audit-v2/run_audit.py):
  ...
  audit_in_process(preset, scene, source_url)
    ...
    downloads source PLY, runs `catetus optimize --preset <p>` then
    `catetus convert --to ply`, then runs psnr_v2 on the result.

The container image carries:
  * gsplat 1.5.3 + torch 2.1.2 + CUDA 12.1 (matches psnr_v2 import deps)
  * catetus CLI built from the pinned Catetus ref (so optimize +
...
# catetus repo at runtime (the catetus clone in the image is only
...
image = (
    ...
    # Build catetus CLI from source. Same approach as apps/worker/worker.py
    ...
    .run_commands(
        ...
        "--manifest-path /opt/catetus/Cargo.toml "
        "-p catetus-cli",
...
def audit_pair(
    ...
    artifact (e.g. the output of catetus-fcgs / catetus-hosted-
...
def audit_in_process(
    ...
    Pipeline: download source PLY -> catetus optimize -> catetus
    ...
    if rc.returncode != 0:
        return {
            ...
            "error": f"catetus optimize rc={rc.returncode}",
            "stderr_tail": rc.stderr[-2000:],
            "stdout_tail": rc.stdout[-2000:],
    ...
    t_cv = time.time()
    rc2 = subprocess.run(
        ["catetus", "convert", str(glb_path), "--to", "ply", "-o", str(dec_ply)],
        capture_output=True, text=True,
    )
    ...
    if rc2.returncode != 0:
        return {
            "label": preset, "scene": scene,
            "error": f"catetus convert rc={rc2.returncode}",
