...
git log -1 --oneline
echo "=== pnpm install ==="
pnpm install --silent --filter @catetus/viewer 2>&1 | tail -5 || true
echo "=== build viewer ==="
pnpm --filter @catetus/viewer build 2>&1 | tail -10
echo "=== run LODGE bench (Sweet Corals) ==="
export SF_BENCH_PLY_DIR="/mnt/c/Users/monta/Catetus/.bench-scenes"
