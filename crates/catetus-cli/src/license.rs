//! `catetus license …` and `catetus serve` implementations.
//!
//! The license subcommands and the `serve` command live here so the main
//! `catetus` binary stays a thin dispatch shell. None of these handlers
//! ever see the issuer's private key — they only consume the public
//! signature path via `catetus-license`.

use std::path::{Path, PathBuf};
use std::time::Duration as StdDuration;

use anyhow::{anyhow, bail, Context, Result};
use catetus_license::{License, LicenseConfig};
...
pub fn default_license_path() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".catetus").join("license.lic")
    } else {
        PathBuf::from("./catetus.lic")
    }
}
...
/// `catetus license install <path>` — verifies signature, copies file
...
/// `catetus license status` — pretty-print the active license + grace
...
pub fn cmd_license_status(license_override: Option<&Path>) -> Result<()> {
    ...
    let lic = License::read_from_path(&path).with_context(|| {
        format!(
            "no license at {} — run `catetus license install <file>`",
            path.display()
        )
    })?;
    let cfg = LicenseConfig::default();
...
/// `catetus license refresh` — POST the current license to the API,
...
/// `catetus serve` — on-prem optimize service.
///
...
pub fn cmd_serve(
    ...
    let lic = License::read_from_path(&path).with_context(|| {
        format!(
            "no license at {} — run `catetus license install <file>` before `serve`",
            path.display()
        )
    })?;
    let cfg = LicenseConfig::default();
    ...
    let telemetry_enabled = std::env::var("CATETUS_NO_TELEMETRY")
        .map(|v| v != "1")
        .unwrap_or(true);
    eprintln!(
        "catetus serve: org={} seats={} valid_until={} telemetry={}",
        lic.claims.org_id,
        lic.claims.seats,
        lic.claims.valid_until.to_rfc3339(),
    ...
    eprintln!("catetus serve: listening on http://{bind}");
    for req in server.incoming_requests() {
        ...
        let body = if url == "/healthz" {
            ...
            tiny_http::Response::from_string("catetus serve: handler not wired on this branch")
...
mod tests {
    ...
    #[test]
    fn default_path_under_dot_catetus() {
        let p = default_license_path();
        assert!(p.ends_with("license.lic"));
        assert!(p.to_string_lossy().contains(".catetus"));
    }
}
