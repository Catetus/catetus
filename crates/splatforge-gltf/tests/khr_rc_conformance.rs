//! Live integration of `splatforge-gltf::write_glb` / `write_gltf` against
//! the `splatforge-khr-conformance` validator. Locks in that the production
...
use splatforge_gltf::{write_glb, write_gltf, SpecVersion, WriteOpts};
...
fn assert_all_clauses_pass(path: &std::path::Path) {
    ...
    let by_id: HashMap<_, _> = report.clauses.iter().map(|c| (c.id.as_str(), &c.status)).collect();
    for required in [
        "EXT_USED",
        "ASSET_VERSION",
        "PRIM_EXT",
...
fn rc_sh_glb_passes_validator() {
    ...
    assert_eq!(
        report.clauses.iter().filter(|c| matches!(c.status, Status::Fail)).count(),
        0,
...
fn rc_writer_roundtrip_preserves_splats() {
    ...
    let scene = deterministic_scene(6, false);
    write_glb(&scene, &path, &WriteOpts::default()).expect("write");
    let decoded = splatforge_gltf::read_glb(&path).expect("read");
