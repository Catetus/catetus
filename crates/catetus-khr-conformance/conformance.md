...
Catetus ships a self-contained conformance suite for the Khronos
...
[catetus.com/khr-conformance](https://catetus.com/khr-conformance).
...
- The validator is wired as a Cargo integration test, so `cargo test -p
  catetus-khr-conformance` is the canonical pass/fail gate.
...
```
KHR_gaussian_splatting conformance report for crates/catetus-khr-conformance/fixtures/01_valid_baseline.glb (glb)
...
`cargo test -p catetus-khr-conformance` on every PR. The validator
