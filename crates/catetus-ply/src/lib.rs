#![deny(clippy::all)]
//! Inria-style 3DGS PLY → Catetus IR.
//!
...
fn read_binary(bytes: &[u8], header: &Header, vertex_idx: usize) -> Result<Vec<Splat>, PlyError> {
    ...
    if count < PARALLEL_THRESHOLD {
        ...
        for i in 0..count {
            ...
            splats.push(decode_row(
                row,
                &layout,
                sh_degree,
                total_coeffs_per_channel,
            ));
        }
        return Ok(splats);
    }
...
fn logit(p: f32) -> f32 {
    // Inverse of sigmoid. Clamp slightly inside (0, 1) to avoid ±inf. We
    // use a 1e-7 deadzone (matches the previous `f32::EPSILON` clamp's
    // numerical effect) rather than `f32::MIN_POSITIVE` because logit grows
    // very slowly near the tails — anything tighter just chases noise.
...
fn ln_scale(s: f32) -> f32 {
    ...
    // Use `f32::MIN_POSITIVE` (~1.18e-38) so genuinely small (but
    // exp-representable) 3DGS scales like `exp(-19) ≈ 5.6e-9` survive the
    // round-trip — the prior `f32::EPSILON` (1.19e-7) clamp was the
    // remaining source of the -15.94 floor seen in pre-fix `_sf_FIXED.ply`
    // decodes (see experiments/decoder-conventions-fix/RESULT.md).
    let s = s.max(f32::MIN_POSITIVE);
    s.ln()
}
