...
/// One encoded video stream inside a CodecGS-Lite payload.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct CodecGSChannel {
    pub name: String,
    pub mins: Vec<f32>,
    pub maxs: Vec<f32>,
    pub sub_channels: u32,
    pub n_frames: u32,
    pub crf: u32,
    #[serde(skip)]
    pub data: Vec<u8>,
...
/// CodecGS-Lite payload (video-encoded parameter image).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct CodecGSPayload {
    pub codec: String,
    ...
    pub f_rest_dim: u32,
    pub sort: String,
    pub channels: Vec<CodecGSChannel>,
    pub version: String,
    pub xyz_log_transform: bool,
}
...
pub struct SplatScene {
    ...
    /// Optional CodecGS-Lite payload (video-encoded parameter image). When
    /// `Some`, the writer can emit a self-contained GLB via the
    /// `SF_codecgs_lite` extension. Backward compatible — defaults to `None`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub codecgs: Option<CodecGSPayload>,
}
