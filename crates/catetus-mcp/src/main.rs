#![deny(clippy::all)]
//! `catetus-mcp` — Model Context Protocol server for Catetus.
//!
...
//!
//!   - `catetus.analyze`      — wraps [`AnalyzeReport::from_scene`]
//!   - `catetus.list_presets` — static catalog of built-in presets
//!   - `catetus.optimize`     — runs a free preset through the Pipeline
//!
//! The Streamable-HTTP transport, the authenticated tier (which gates the
//! `catetus.repack` paid preset behind an API key), and the moat tools
...
use catetus_core::{format_from_extension, format_from_magic, AnalyzeReport, SplatScene};
use catetus_gltf::{read_glb, read_gltf, write_gltf, WriteOpts};
use catetus_optimize::preset;
use catetus_ply::{read_ply, write_ply};
use catetus_spz::read_spz;
...
const TOOLS: &[ToolDef] = &[
    ...
    ToolDef {
        name: "catetus.list_presets",
        description: "List all built-in Catetus presets with their target use case, typical \
    ...
    ToolDef {
        name: "catetus.optimize",
        description: "Run a Catetus preset over a splat file and return the output path plus the \
    ...
    ToolDef {
        name: "catetus.generate_lod_pyramid",
        description: "Take one .ply / .spz / .gltf and emit N .ply files at user-specified \
             ...
             4 .ply files at decreasing detail) — Catetus generates the pyramid \
             from a single converged scene in a few seconds.",
        input_schema: schema_generate_lod_pyramid,
    },
    ToolDef {
        name: "catetus.find_similar_scenes",
...
fn call_tool(name: &str, args: &Value) -> ToolResult {
    match name {
        "catetus.analyze" => tool_analyze(args),
        "catetus.list_presets" => tool_list_presets(),
        "catetus.optimize" => tool_optimize(args),
        "catetus.generate_lod_pyramid" => tool_generate_lod_pyramid(args),
        "catetus.find_similar_scenes" => tool_find_similar_scenes(args),
        other => tool_error(format!("unknown tool: {other}")),
    }
}

// ---------------------------- catetus.generate_lod_pyramid handler
...
// --------------------------------- catetus.find_similar_scenes handler
...
fn tool_find_similar_scenes(args: &Value) -> ToolResult {
    ...
    if corpus.is_empty() {
        return tool_error(
            "bundled SplatBench corpus is empty — rebuild catetus-mcp \
    ...
    presets.push(json!({
        ...
        "description":
            "Self-distillation pass on an A100 — render the scene from 32 pseudo \
             ...
             authenticated MCP tier (set CATETUS_API_KEY and use \
             `catetus.repack` instead of this tool).",
        "typicalRatio": 22.0,
        "bestFor": ["maximum compression with measurable fidelity"],
    }));
    let text = format!(
        "{} free presets and 1 paid preset available. Use a free preset with \
         `catetus.optimize`; the paid preset requires the authenticated tier.",
        PRESET_INFO.len()
    );
    ToolResult {
    ...
    if preset_name == "differentiable-repack" {
        return tool_error(
            "preset 'differentiable-repack' is paid and requires the authenticated tier. \
             Use CATETUS_API_KEY + the `catetus.repack` tool on the hosted MCP."
                .to_string(),
        );
    }
    let chunked = args
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "catetus_mcp=info".into()),
        )
        .with_writer(std::io::stderr)
        .try_init();
    tracing::info!(version = env!("CARGO_PKG_VERSION"), "catetus-mcp ready");
...
mod tests {
    ...
    fn initialize_advertises_tools_capability() {
        ...
        assert_eq!(result["serverInfo"]["name"], "catetus-mcp");
    ...
    fn tools_list_returns_public_tier_tools() {
        ...
        assert!(names.contains(&"catetus.analyze"));
        assert!(names.contains(&"catetus.list_presets"));
        assert!(names.contains(&"catetus.optimize"));
    }
    ...
    fn list_presets_includes_paid_preset_marker() {
        let resp = dispatch(req(
            3,
            "tools/call",
            json!({ "name": "catetus.list_presets", "arguments": {} }),
        ));
    ...
    fn unknown_method_returns_method_not_found() {
        let resp = dispatch(req(6, "catetus/teleport", json!({})));
