...
SplatForge ships a self-contained conformance suite for the OpenUSD 26.03
...
- The validator is also wired as a Cargo integration test, so
  `cargo test -p splatforge-usd-conformance` is the canonical pass/fail gate.
...
cargo test -p splatforge-usd-conformance
```
...
| `SH_COEFFS_COUNT`           | MUST     | When authored, `custom float[] splatforge:shCoefficients` count MUST equal `splat_count * (degree+1)^2 * 3` for `degree ∈ {0,1,2,3}`.             |
...
| `04_valid_with_sh.usda`                    | USDA      | Adds `custom float[] splatforge:shCoefficients` (degree 3); `SH_COEFFS_COUNT` PASS. |
...
```
ParticleField3DGaussianSplat conformance report for crates/splatforge-usd-conformance/fixtures/01_valid_minimal.usda (usda)
...
`crates/splatforge-usd/SPEC-GAPS.md` — and the choices are surfaced as
...
`cargo test -p splatforge-usd-conformance` on every PR. The validator
