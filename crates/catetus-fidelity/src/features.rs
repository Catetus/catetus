...
/// Reduce a `SplatScene` (already parsed via `catetus-ply::read_ply`) to
...
pub fn summarise(scene: &SplatScene) -> Summary {
    ...
    // scale stats — catetus-ply already decoded log-scale.
    ...
    // opacity (already in [0,1] from catetus-ply)
    ...
    // color: catetus-ply emits Color::Sh{...} for Inria PLYs with f_rest_*
