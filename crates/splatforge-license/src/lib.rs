#![deny(clippy::all)]
//! `splatforge-license` — Ed25519-signed license file format + verifier for
//! the SplatForge Pro on-prem tier.
//!
//! ## File format
//!
//! `splatforge.lic` is plain UTF-8 JSON with a stable shape:
//!
//! ```json
//! {
//!   "org_id": "acme-corp",
...
//! A `splatforge` binary built from this repo will only honor licenses
...
//!   `splatforge serve` calls so a customer can survive a 7-day API
...
/// Default offline-grace window. Customers running `splatforge serve`
...
/// CLI exit code so `splatforge license status` can report a useful
/// reason.
#[derive(Debug, Error)]
...
impl License {
    ...
    /// was within `grace`. The predicate `splatforge serve` calls on
    /// startup.
...
mod tests {
    ...
    fn write_and_read_round_trip() {
        ...
        let path = tmp.path().join("splatforge.lic");
