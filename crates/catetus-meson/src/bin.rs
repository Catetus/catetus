...
use catetus_meson::{decode_features, decode_scene, encode_scene, EncodeConfig, MesonError};
...
fn run() -> Result<(), MesonError> {
    ...
    match cli.cmd {
        Cmd::Encode {
            ...
            let t0 = Instant::now();
            let scene = catetus_ply::read_ply(&input)?;
        ...
        Cmd::Decode { input, output } => {
            ...
            catetus_ply::write_ply(&scene, &output)?;
            println!("decoded {} splats in {:.2}s", scene.splats.len(), dec_secs);
            Ok(())
        }
        Cmd::Info { input } => {
            let bytes = fs::read(&input)?;
            let layout = catetus_meson::container_layout(&bytes)?;
