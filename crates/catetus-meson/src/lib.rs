...
impl Features {
    ...
    pub fn to_scene(&self) -> SplatScene {
        ...
        for i in 0..self.n {
            ...
            splats.push(catetus_core::Splat {
                position,
        ...
        SplatScene {
            splats,
            coordinate_system: catetus_core::CoordinateSystem::default(),
...
pub fn quantize_group(
    ...
    let stats: Vec<ChannelStats> = (0..d)
        .map(|ch| build_channel_stats(data, n, d, ch))
        .collect();
...
pub fn encode_xyz(xyz: &[f32], bbox: &[[f32; 3]; 2], bits: u8) -> Vec<u8> {
    ...
    for i in 0..n {
        for a in 0..3 {
            let q =
                ((xyz[3 * i + a] - bbox[0][a]) * inv_span[a]).clamp(0.0, max_code as f32) as u32;
...
fn write_container(
    ...
    let mut buf: Vec<u8> =
        Vec::with_capacity(meta_bytes.len() + streams.iter().map(|s| s.len()).sum::<usize>() + 256);
    buf.write_all(MAGIC)?;
