...
fn find_first_prediction_divergence() {
    let Ok(pthc) = env::var("SPLATFORGE_PYTHON_PTHC") else { return; };
    let Ok(preds) = env::var("SPLATFORGE_PY_ALLPREDS") else { return; };
    ...
    for i in 0..n {
        let p = [
            ...
            (positions[3*i+1] - header.pos_mn[1]) / (header.pos_mx[1] - header.pos_mn[1]).max(1e-9),
            (positions[3*i+2] - header.pos_mn[2]) / (header.pos_mx[2] - header.pos_mn[2]).max(1e-9),
        ];
        let pred = predict(p, &header.config, &weights);
        for c in 0..d {
            let mr = pred.mean[c]; let sr = pred.std[c];
            let mp = py_mu[i*d + c]; let sp = py_sigma[i*d + c];
            if (mr as f32 - mp).abs() > 1e-4 || (sr as f32 - sp).abs() > 1e-4 {
                first_diverge = Some((i, c, mr, mp, sr, sp));
                break;
            }
        }
        if first_diverge.is_some() { break; }
    }
    match first_diverge {
