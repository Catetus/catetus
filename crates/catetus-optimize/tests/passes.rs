use catetus_core::{Color, SemanticLabel, Splat, SplatScene};
...
fn floater_prune_noop_below_k() {
    ...
    let mut scene = SplatScene::new();
    for i in 0..5 {
        scene.splats.push(splat_at([i as f32 * 0.1, 0.0, 0.0]));
    }
    let mut ctx = PassContext::default();
    let stats = FloaterPrune::default().run(&mut scene, &mut ctx).unwrap();
    ...
    for i in 0..n {
        for j in 0..n {
            for k in 0..n {
                scene
                    .splats
                    .push(splat_at([i as f32 * 0.1, j as f32 * 0.1, k as f32 * 0.1]));
            }
        }
    }
    let before = scene.splats.len();
