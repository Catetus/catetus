...
//! in PixarAnimationStudios/OpenUSD), the SPEC-0011/0012 Catetus
...
//! file (decoded via [`catetus_usd::read_usdc`] then re-emitted as
...
//! integration tests, the `catetus-usd-validate` CLI binary, and the
//! `catetus spec-check` shell-out from the top-level CLI.
//!
//! Where the schema is ambiguous (see `crates/catetus-usd/SPEC-GAPS.md`)
...
impl Clause {
    ...
    pub fn description(self) -> &'static str {
        match self {
            ...
            Clause::ShCoefficientsCount => {
                "When authored, `custom float[] catetus:shCoefficients` count MUST equal \
...
pub fn validate_path(path: &Path) -> Result<Report, ValidateError> {
    ...
    let (text, container) = match ext.as_str() {
        ...
        "usdc" => {
            ...
            let scene = catetus_usd::read_usdc(path)?;
            let text = catetus_usd::render_usda(&scene, &catetus_usd::UsdWriteOpts::default());
            (text, "usdc")
        }
...
/// `catetus-usd`. We only re-derive enough structure to answer the
/// clause questions.
struct ParsedDoc<'a> {
    /// Raw text body.
    raw: &'a str,
    ...
    /// `custom float[] catetus:shCoefficients` array, if present.
...
fn parse_doc(raw: &str) -> ParsedDoc<'_> {
    ...
    let sh_coefficients = pull_scalar_array(raw, &["custom float[] catetus:shCoefficients"]);
...
fn typed_presence(raw: &str, attr_name: &str, want_type: &str) -> Option<bool> {
    ...
    // Variant: the catetus USDA writer never authors attribute keys with
