//! `catetus-usd-fixtures` — deterministic generator for the
//! `ParticleField3DGaussianSplat` conformance fixture corpus.
//!
//! Usage:
//!     catetus-usd-fixtures <out_dir>
//!
...
//!   04_valid_with_sh.usda          adds `custom float[] catetus:shCoefficients` (degree 3)
...
//! USDA output from `catetus_usd::render_usda`, not by hand-crafting
...
use catetus_usd::{render_usda, write_usda, write_usdc, UsdWriteOpts};
...
fn three_splat_scene() -> SplatScene {
    let mut scene = SplatScene::new();
    // Use the same three splats from catetus-usd/fixtures/particle_field.usda
