#!/usr/bin/env bash
# SplatForge end-to-end CLI happy path.
#
...
#   SPLATFORGE_BIN=/path/to/splatforge ./tests/integration/cli.sh
...
if [[ ! -x "$BIN" ]]; then
  ...
  echo "       build it first: cargo build --release -p splatforge-cli" >&2
  exit 127
fi
