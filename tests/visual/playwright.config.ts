/**
 * Playwright configuration for the Catetus visual-diff (SPEC-0009) and
 ...
 * page reads `renderer` and constructs a `CatetusViewer` with the matching
 * `renderer` option, deterministic=true and cameraPath='orbit-8'.
 */
import { defineConfig, devices } from '@playwright/test';

const PORT = Number(process.env.CATETUS_HARNESS_PORT ?? 4317);
