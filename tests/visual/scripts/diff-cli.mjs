#!/usr/bin/env node
/**
 * `splatforge diff` helper (SPEC-0009).
 *
...
/**
 * If `path` is a `.ply` or `.spz`, shell out to the splatforge binary to
...
function ensureGltf(path, tmpDir, cli) {
  ...
  if (!cli) {
    throw new Error('cannot convert without a --cli splatforge binary path');
  }
  ...
  if (res.status !== 0) {
    throw new Error(`splatforge convert failed for ${path} (exit ${res.status})`);
  }
  return out;
}
...
/**
 * Render `diff.html`. Prefers `@splatforge/report-ui` if available (built
...
async function renderReportHtml(data) {
  ...
  try {
    const mod = await import('@splatforge/report-ui');
...
<title>SplatForge diff — ${esc(data.asset || 'report')}</title>
<style>body{font-family:ui-monospace,Menlo,monospace;background:#0b0d10;color:#d7dde4;padding:24px}</style>
</head><body>
<h1>SplatForge visual diff — ${esc(data.asset || 'report')} (${esc(status.toUpperCase())})</h1>
${banner}
...
async function emitReport(outDir, payload) {
  ...
  if (process.env.SPLATFORGE_INCLUDE_TIMESTAMPS === '1') {
...
async function main() {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'splatforge-diff-'));
