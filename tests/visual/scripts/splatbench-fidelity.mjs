...
const CLI = resolve(ROOT, 'target/release/catetus');
...
function inputPlyBbox(ply) {
  ...
  if (res.status !== 0) {
    throw new Error(`catetus analyze ${ply} exited with status ${res.status}`);
  }
...
async function main() {
  if (!existsSync(CLI)) {
    throw new Error(`catetus binary not found at ${CLI}. Run cargo build --release first.`);
  }
  ...
  if (!results) {
    results = {
      schema: 'catetus.splatbench.fidelity/0.1',
      name: 'SplatBench v0 — fidelity',
      catetusVersion: '0.1.1',
