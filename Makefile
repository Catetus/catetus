# SplatForge convenience targets.
#
...
BIN ?= target/release/splatforge
TINY ?= fixtures/tiny/basic_binary.ply
DEMO_OUT ?= /tmp/splatforge-demo
...
.PHONY: build-cli
build-cli: ## Build the splatforge CLI (release)
...
install-playwright: ## Install playwright-core + chromium under tests/visual/
	...
	pnpm -F splatforge-visual exec playwright install --with-deps chromium

.PHONY: diff-tiny
diff-tiny: build-cli ## Run splatforge diff on the tiny fixture (degrades gracefully without playwright)
	$(BIN) diff $(TINY) $(TINY) --out /tmp/splatforge-diff
	@echo "diff written to /tmp/splatforge-diff"
