...
```
cargo build --release -p catetus-cli    # → target/release/catetus
...
```
Catetus/
├── target/release/catetus      # Rust CLI binary (~2 MB)
├── packages/viewer/dist/          # @catetus/viewer ESM build
├── packages/report-ui/dist/       # @catetus/report-ui ESM build
└── tests/visual/node_modules/     # Playwright deps (if --F catetus-visual install ran)
```

## Verifying the install

```bash
./target/release/catetus --help
...
./target/release/catetus inspect /tmp/out.gltf
```
...
./target/release/catetus diff before.ply after.gltf --out reports/diff/
open reports/diff/diff.html
```

If `playwright-core` is not installed, `catetus diff` produces a valid but
...
This happens on some FUSE-mounted directories (Dropbox, iCloud, sandboxed mounts). Build in a regular filesystem location or set `CARGO_TARGET_DIR=$HOME/.catetus-target`.

### WebGPU not available in your browser
`@catetus/viewer` auto-falls-back to WebGL2. Force the renderer with `new CatetusViewer({ renderer: 'webgl2', ... })`.

### Playwright browsers fail to install
Run `pnpm -F catetus-visual exec playwright install --with-deps chromium` to also install OS-level prerequisites.
...
Run `catetus inspect your.ply` first. Most issues are missing required fields (`rot_*`, `scale_*`, `f_dc_*`) or big-endian PLYs (not yet supported — see [SPEC-0002](./specs/0002-ply-ingest.md)).
