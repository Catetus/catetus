...
// NB: importing QATPlyDataSource requires Cesium, which pulls in DOM globals.
// We dynamic-import it inside the test that needs it to keep header/dequant
// tests pure-codec (no global-state side-effects).
import {
  buildSyntheticFixture,
  floatToHex,
  floatArrayToBase64,
} from "../../fixtures/buildFixture.js";
...
describe("synthetic QAT-PLY round-trip (Cesium edition)", () => {
...
describe("QATPlyDataSource.decode end-to-end", () => {
  it("returns positions + colors + decoded quantized fields", async () => {
    const { QATPlyDataSource } = await import("../src/QATPlyDataSource.js");
    const fx = buildSyntheticFixture(16);
    const ds = new QATPlyDataSource("test");
    const result = ds.decode(fx.bytes);
    expect(result.vertexCount).toBe(16);
    expect(result.positions.length).toBe(48);
    expect(result.colors?.length).toBe(48);
    expect(result.anchorFeat?.channels).toBe(4);
    expect(result.offset?.channels).toBe(6);
    expect(Array.from(result.positions)).toEqual(Array.from(fx.expectedPositions));
    expect(Array.from(result.anchorFeat!.data)).toEqual(Array.from(fx.expectedAnchorFeat));
    expect(Array.from(result.offset!.data)).toEqual(Array.from(fx.expectedOffset));
  });

  it("rejects truncated bodies with a clear error", async () => {
    const { QATPlyDataSource } = await import("../src/QATPlyDataSource.js");
    const fx = buildSyntheticFixture(8);
    const truncated = fx.bytes.subarray(0, fx.bytes.length - 4);
    const ds = new QATPlyDataSource();
    expect(() => ds.decode(truncated)).toThrow(/truncated/);
  });
});
