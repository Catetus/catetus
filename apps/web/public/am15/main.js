...
let viewMatrix = defaultViewMatrix; window.viewMatrix = viewMatrix;
async function main() {
    let carousel = true; if (window.__noCarousel) carousel = false;
    const params = new URLSearchParams(location.search);
    try {
        viewMatrix = JSON.parse(decodeURIComponent(location.hash.slice(1))); window.viewMatrix = viewMatrix;
        carousel = false;
    } catch (err) {}
    const url = new URL(
        // "nike.splat",
        ...
        (params.get("local") === "1" ? location.origin + "/" : (params.get("local") === "0" ? location.origin + "/" : "https://huggingface.co/cakewalk/splat-data/resolve/main/")),
    );
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (/\d/.test(e.key)) {
            ...
            viewMatrix = getViewMatrix(camera); window.viewMatrix = viewMatrix;
        }
        if (["-", "_"].includes(e.key)) {
            currentCameraIndex =
                (currentCameraIndex + cameras.length - 1) % cameras.length;
            viewMatrix = getViewMatrix(cameras[currentCameraIndex]); window.viewMatrix = viewMatrix;
        }
        if (["+", "="].includes(e.key)) {
    ...
    window.addEventListener(
        ...
        (e) => {
            ...
            viewMatrix = invert4(inv); window.viewMatrix = viewMatrix;
        },
        { passive: false },
    );
    ...
    const frame = (now) => {
        // Hero orbit override: external code can set window.__heroOrbit to a 16-float viewMatrix
        if (window.__heroOrbit) { viewMatrix = window.__heroOrbit; carousel = false; }
        let inv = invert4(viewMatrix);
        let shiftKey =
    ...
    const selectFile = (file) => {
        ...
        if (/\.json$/i.test(file.name)) {
            fr.onload = () => {
                ...
                viewMatrix = getViewMatrix(cameras[0]); window.viewMatrix = viewMatrix;
                projectionMatrix = getProjectionMatrix(
