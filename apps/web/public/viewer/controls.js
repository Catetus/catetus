import { frameCameraToBbox } from './camera-state.js';
export class OrbitControls {
    ...
    onPointerDown = (e) => {
        ...
        // Match SuperSplat / antimatter15 default: L-drag = free-look (camera
        // position fixed, look direction rotates) — feels like "swinging the
        // view" rather than "swinging around an orbit pivot." Orbit is still
        // available behind Alt+L for users who want the bbox-pivot mode.
        // Trackpad-friendly modifier fallbacks (priority: Alt > Shift > base).
        this.dragging = right || (left && e.altKey); // orbit
        this.panning = !this.dragging && (middle || (left && e.shiftKey));
        this.freelook = !this.dragging && !this.panning && left; // default L-drag
        this.lastPos = { x: e.clientX, y: e.clientY };
    };
    onPointerMove = (e) => {
        if (e.pointerType === 'touch')
            return this.handleTouchMove(e);
        if (!this.dragging && !this.panning && !this.freelook)
            return;
        const dx = e.clientX - this.lastPos.x;
        const dy = e.clientY - this.lastPos.y;
        this.lastPos = { x: e.clientX, y: e.clientY };
        if (this.freelook)
            this.mouseLook(dx, dy);
     ...
     */
    pickAndRetarget(canvasPx, canvasPy, w, h) {
    ...
    applyKeyboard(dtMs) {
        ...
        let pX = 0, pZ = 0;
        if (this.keys.has('w'))
            pZ -= 1; // forward (along view direction)
        if (this.keys.has('s'))
            pZ += 1; // back
        if (this.keys.has('a'))
            pX += 1; // strafe LEFT
