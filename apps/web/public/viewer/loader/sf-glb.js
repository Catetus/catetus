 ...
 * the two.
 */
import { applyV5TailToScene, decodeSFExtensions, decodeV5TailBytes, } from '@catetus/glb-polyfill';
import { clamp01, computeBbox, normalizeQuatInto, SH_C0, shRestCoefCount, } from './splat-scene.js';
/** Split a GLB blob into its JSON + BIN chunks. */
function parseGlb(buf) {
    const dv = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
    const magic = dv.getUint32(0, true);
    if (magic !== 0x46546C67)
        throw new Error('glb: bad magic'); // 'glTF'
    const version = dv.getUint32(4, true);
    if (version !== 2)
        throw new Error(`glb: unsupported version ${version}`);
    let cursor = 12;
    let json = null;
    let bin = null;
...
export async function loadSfGlb(buf, sourceName, opts = {}) {
    ...
    const palExt = ext['CT_gaussian_splatting_palette'];
    ...
    if (palExt?.uri && !(palExt.uri in sidecars)) {
        ...
        if (!res.ok)
            throw new Error(`sf-glb: failed to fetch sidecar ${url}: HTTP ${res.status}`);
    ...
    let shRest = decoded.sh_rest ?? undefined;
    let shDegree = decoded.shDegree > 0 ? decoded.shDegree : undefined;
    const extras = {};
    if (decoded.extensionsApplied.palette)
        extras.palette = 'SF';
    if (decoded.extensionsApplied.zstdSplitBuffer)
        extras.zstd = 'split';
    if (decoded.extensionsApplied.smallest3)
        extras.quat = 'smallest3';
...
//# sourceMappingURL=sf-glb.js.map