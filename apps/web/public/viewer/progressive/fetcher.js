 ...
 *
 * Header layout (per `crates/catetus-ply/src/progressive.rs`):
 *
 *   [4]  magic           = b"MGS2"
 *   [4]  version u32     = 1
 *   [4]  flags   u32     = 0
 ...
 */
export function parsePlyHeader(headerBytes) {
...
class ByteAccumulator {
    ...
    take(n) {
        if (n > this.total)
            throw new Error(`ByteAccumulator: take(${n}) > length(${this.total})`);
...
/* ------------------------------------------------------------------------ */
async function* readableStreamToAsyncIter(reader, signal) {
    try {
        for (;;) {
            if (signal?.aborted)
                break;
 ...
 */
export async function* fetchProgressive(url, opts = {}) {
    ...
    if (opts.source) {
        ...
        const body = res.body;
        if (!body)
            throw new Error('mgs2_fetch_failed: response has no body');
    ...
    while (acc.length < MGS2_PREFIX_LEN) {
        ...
        if (!got) {
            throw new Error(`mgs2_truncated_header: only ${acc.length} of ${MGS2_PREFIX_LEN} prefix bytes received`);
        }
    ...
    for (;;) {
        if (acc.length >= MGS2_PREFIX_LEN && !header) {
            ...
            for (let i = 0; i < 4; i++) {
                if (prefix[i] !== MGS2_MAGIC[i]) {
                    throw new Error(`mgs2_bad_magic: expected "MGS2", got "${new TextDecoder('ascii').decode(prefix.subarray(0, 4))}"`);
                }
            }
            ...
            while (acc.length < plyHeaderLen) {
                const got = await pullMore();
                if (!got) {
                    throw new Error(`mgs2_truncated_ply_header: ${acc.length} of ${plyHeaderLen} bytes received`);
                }
                if (opts.signal?.aborted)
                    return;
            }
            ...
            if (fieldOffsets.recordSize !== recordSize) {
                throw new Error(`mgs2_record_size_mismatch: header says ${recordSize}, PLY columns sum to ${fieldOffsets.recordSize}`);
            }
        ...
        if (header && acc.length >= recordSize) {
            ...
            if (records > 0) {
                ...
                const recsToEmit = Math.min(records, Math.floor(batchBytes / recordSize) || 1, remaining);
                if (recsToEmit > 0) {
