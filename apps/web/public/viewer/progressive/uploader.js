 ...
 */
export function buildSoaChunk(records, count, off) {
    if (count * off.recordSize !== records.byteLength) {
        throw new Error(`progressive uploader: records (${records.byteLength} B) is not a whole number of records (count=${count}, stride=${off.recordSize})`);
    }
 ...
 */
function makeBatchDescriptor(index, count, layout, bbox, byteLength) {
    return {
...
export class ProgressiveUploader {
    ...
    addBatch(records, count) {
        ...
        const descriptor = makeBatchDescriptor(this.batchIndex, count, layout, bbox, bytes.byteLength);
        this.renderer.uploadChunk(descriptor, bytes);
        if (this.keepDecoded) {
            const decoded = decodeChunkBytes(bytes, descriptor);
            for (const s of decoded)
                this.decodedSplats.push(s);
        }
        // Union bbox.
        if (count > 0) {
            if (bbox.min[0] < this.bboxMin[0])
                this.bboxMin[0] = bbox.min[0];
            if (bbox.min[1] < this.bboxMin[1])
                this.bboxMin[1] = bbox.min[1];
            if (bbox.min[2] < this.bboxMin[2])
                this.bboxMin[2] = bbox.min[2];
            if (bbox.max[0] > this.bboxMax[0])
                this.bboxMax[0] = bbox.max[0];
            if (bbox.max[1] > this.bboxMax[1])
                this.bboxMax[1] = bbox.max[1];
            if (bbox.max[2] > this.bboxMax[2])
                this.bboxMax[2] = bbox.max[2];
        }
