...
export class WebGPURenderer {
    ...
    /**
     * GPU readback of the rendered color image into a CPU `Uint8Array` of RGBA
     * bytes (row-major, top-left origin), bypassing the swapchain entirely.
     *
     * Headless CDP `Page.captureScreenshot` returns BLACK for the WebGPU canvas
     * on this box, so visual verification must read the GPU result directly. We
     * re-run the SAME compute-decode draw path used by `renderFrame`, but target
     * an offscreen `rgba8unorm` texture flagged `COPY_SRC`, then
     * `copyTextureToBuffer` → `mapAsync` → copy out. The format is forced to
     * `rgba8unorm` (not the canvas's `bgra8unorm`) so the returned bytes are in
     * canonical R,G,B,A order regardless of platform swizzle.
     *
     * @returns `{ width, height, rgba }`.
     */
    async readbackRGBA(camera) {
        if (!this.device || !this.pipeline || !this.uniformBuffer || !this.bindGroup) {
            throw new Error('renderer_init_failed: not initialized');
        }
        const canvas = this.canvas;
        if (!canvas)
        ...
        const focalY = height / (2 * Math.tan(camera.fovY * 0.5));
        const focalX = focalY;
        // Offscreen color target in canonical RGBA order, copyable to a buffer.
        // We build a dedicated rgba8unorm pipeline lazily — the cached
        // `this.pipeline` targets the canvas's preferred format (bgra on macOS),
        // and a render pipeline's fragment target format must match the
        // attachment, so reusing it against an rgba8unorm texture would error.
        const target = this.device.createTexture({
            size: { width, height },
            format: 'rgba8unorm',
            usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.COPY_SRC,
        });
        if (!this._readbackPipeline) {
            const module = this.device.createShaderModule({ code: WGSL });
            const bindGroupLayout = this.device.createBindGroupLayout({
                entries: [{ binding: 0, visibility: GPUShaderStage.VERTEX, buffer: { type: 'uniform' } }],
            });
            const layout = this.device.createPipelineLayout({ bindGroupLayouts: [bindGroupLayout] });
            this._readbackPipeline = this.device.createRenderPipeline({
                layout,
                vertex: {
                    module,
                    entryPoint: 'vs_main',
        ...
        const pipeline = this._readbackPipeline;
        // Uniform: viewport size.
        const u = new Float32Array(4);
        u[0] = width;
        u[1] = height;
        this.device.queue.writeBuffer(this.uniformBuffer, 0, u.buffer, u.byteOffset, u.byteLength);
        const encoder = this.device.createCommandEncoder();
        // Compute-decode path (the live tileset + single-file viewer path).
        if (this.compute && this.options.useComputeDecode) {
            const count = this.compute.splatCount;
            this.compute.encode(encoder, view, viewProj, [focalX, focalY], [width, height], [camera.position[0], camera.position[1], camera.position[2]]);
            const pass = encoder.beginRenderPass({
            ...
            pass.setBindGroup(0, this.bindGroup);
            if (count > 0) {
                for (const page of this.compute.instancePages) {
                    const pageCount = Math.min(page.splatCount, count - page.splatStart);
                    if (pageCount <= 0)
                        break;
                    pass.setVertexBuffer(0, page.buffer);
            ...
            // CPU path: rely on the most recent renderFrame having populated the
            // instance buffer; redraw it into the offscreen target.
            const pass = encoder.beginRenderPass({
                colorAttachments: [{
                    view: target.createView(),
                    clearValue: { r: this.clear[0], g: this.clear[1], b: this.clear[2], a: this.clear[3] },
            ...
            pass.setPipeline(pipeline);
            pass.setBindGroup(0, this.bindGroup);
            if (this.instanceCapacity > 0 && this.instanceBuffer) {
                pass.setVertexBuffer(0, this.instanceBuffer);
                pass.draw(VERTICES_PER_QUAD, this.instanceCapacity, 0, 0);
            }
            pass.end();
        }
        // Copy the offscreen color texture into a mappable buffer. bytesPerRow
        // must be a multiple of 256, so pad and unpad on the CPU side.
        ...
        encoder.copyTextureToBuffer({ texture: target }, { buffer: readbackBuffer, bytesPerRow: paddedBytesPerRow, rowsPerImage: height }, { width, height });
        this.device.queue.submit([encoder.finish()]);
        await this.device.queue.onSubmittedWorkDone?.();
        await readbackBuffer.mapAsync(GPUMapMode.READ);
        const padded = new Uint8Array(readbackBuffer.getMappedRange().slice(0));
        readbackBuffer.unmap();
        readbackBuffer.destroy();
        target.destroy();
        // Unpad rows into a tight RGBA buffer.
        const rgba = new Uint8Array(unpaddedBytesPerRow * height);
        for (let y = 0; y < height; y++) {
            rgba.set(padded.subarray(y * paddedBytesPerRow, y * paddedBytesPerRow + unpaddedBytesPerRow), y * unpaddedBytesPerRow);
        }
