...
export function createWSRTilePipelines(device) {
...
export class WSRTilePipeline {
     ...
     */
    encode(encoder, view, viewProj, focal, viewport, splatCount, sigma, bgColor = [0, 0, 0, WSR_TILE_DEFAULT_BG_WEIGHT], vDefault = 0) {
        const w = viewport[0];
        const h = viewport[1];
        if (w <= 0 || h <= 0)
            return;
        if (w > this.maxWidth || h > this.maxHeight) {
            throw new Error(`WSRTilePipeline: viewport ${w}×${h} exceeds capacity ${this.maxWidth}×${this.maxHeight}`);
        }
        this.lastWidth = w;
        this.lastHeight = h;
...
//# sourceMappingURL=wsr_tile.js.map