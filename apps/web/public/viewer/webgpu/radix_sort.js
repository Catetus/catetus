 ...
 */
export function adapterSupportsSubgroups(adapter) {
    ...
    return adapter.features.has('subgroups');
}
/**
 ...
 */
export function createRadixSortPipelines(device, wgslSource, multiBlockScanWgsl = '', histogramSubgroupWgsl = '', radixMergeWgsl = '') {
    ...
    if (multiBlockScanWgsl.length > 0) {
        ...
        const mbMk = (entryPoint) => device.createComputePipeline({ layout: mbLayout, compute: { module: mbModule, entryPoint } });
        scanMb = {
...
export class RadixSort {
    ...
    mergeBindGroupAtoB;
    mergeBindGroupBtoA;
    constructor(device, capacity, pipes, options = {}) {
        this.device = device;
        ...
        if (pipes.merge) {
            ...
            for (let i = 0; i < MAX_CONCURRENT_MERGES; i++) {
                ...
                a2b.push(device.createBindGroup({
                ...
                b2a.push(device.createBindGroup({
                    layout: pipes.merge.bindGroupLayout,
                    entries: [
     ...
     */
    encodeTimed(encoder, count, querySet, baseIndex) {
        if (count <= 1)
            return;
     ...
     */
    encodeTimedDrilled(encoder, count, querySet, baseIndex) {
        if (count <= 1)
            return;
        if (count > this.capacity) {
            throw new Error(`RadixSort: count ${count} exceeds capacity ${this.capacity}`);
    ...
    encode(encoder, count) {
        ...
        if (!this.pipes.merge) {
            throw new Error(`RadixSort: count ${count} exceeds dispatch cap ${SPLAT_DISPATCH_CAP} but ` +
        ...
        while (curRuns.length > 1) {
            ...
            for (let i = 0; i < curRuns.length; i += 2) {
                ...
                this.encodeMerge(encoder, bgPool[slot % bgPool.length], this.mergeUniformPool[slot % this.mergeUniformPool.length], count, mergedBase, mergedLen, a.offset, a.len, b.offset, b.len);
     ...
     */
    encodeChunkInPlace(encoder, chunkOffset, chunkCount) {
        if (chunkCount <= 1) {
        ...
        if (numWgs > WEBGPU_MAX_DISPATCH_PER_DIM) {
            throw new Error(`RadixSort: chunkCount ${chunkCount} would dispatch ${numWgs} workgroups > 65535`);
        }
     ...
     */
    encodeMerge(encoder, bindGroup, uniformBuffer, totalCount, mergedBase, mergedLen, runAStart, runALen, runBStart, runBLen) {
        if (mergedLen === 0)
            return;
        if (!this.pipes.merge) {
    ...
    destroy() {
        ...
        for (const u of this.mergeUniformPool)
            u.destroy();
    }
}
