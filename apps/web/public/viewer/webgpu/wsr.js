...
export function createWSRPipelines(device) {
    ...
    const mk = (mod, bgl, entry) => device.createComputePipeline({
...
export class WSRPipeline {
     ...
     */
    encode(encoder, view, viewProj, focal, viewport, splatCount, sigma, bgColor = [0, 0, 0, WSR_DEFAULT_BG_WEIGHT], vDefault = 0) {
        const w = viewport[0];
        const h = viewport[1];
        if (w <= 0 || h <= 0)
            return;
        if (w > this.maxWidth || h > this.maxHeight) {
            throw new Error(`WSRPipeline: viewport ${w}×${h} exceeds capacity ${this.maxWidth}×${this.maxHeight}`);
        }
        this.lastWidth = w;
        this.lastHeight = h;
        // ---- Clear uniforms. ----
        ...
        if (splatCount > 0) {
            void splatWgs;
            dispatchPerSplat(this.device, encoder, this.pipes.accumulate, this.accumulateBindGroup, this.accumulateUniforms, WSR_ACCUMULATE_UNIFORM_CHUNK_OFFSET_BYTES, splatCount, WSR_WG);
        }
        // ---- cs_wsr_resolve ----
        {
            const pass = encoder.beginComputePass();
