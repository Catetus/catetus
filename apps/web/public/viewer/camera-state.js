...
export function frameCameraToBbox(cam, bbox) {
 ...
 */
export function tightBbox(positions, clip = 0.02) {
    const N = positions.length / 3;
    if (N === 0)
        return { min: [-1, -1, -1], max: [1, 1, 1] };
    const lo = clip * 0.5;
    const hi = 1 - lo;
    const out = { min: [0, 0, 0], max: [0, 0, 0] };
...
export function viewProjMatrix(cam, aspect) {
...
function lookAt(eye, center, up, roll) {
    ...
    v[4] = sy;
    v[8] = sz;
    v[12] = -(sx * eye[0] + sy * eye[1] + sz * eye[2]);
    v[1] = ux;
    v[5] = uy;
    v[9] = uz;
