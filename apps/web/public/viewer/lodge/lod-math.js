 ...
 */
export function boundaryBlendT(camera, mo, mf) {
    const vx = mf[0] - mo[0];
    const vy = mf[1] - mo[1];
    const vz = mf[2] - mo[2];
 ...
 *      t_blend = 1.
 */
export function selectChunkActivation(record, inputs) {
    ...
    for (let l = 0; l < L; l++) {
        if (distEff >= (inputs.depthThresholds[l] ?? 0))
            chosen = l;
    }
...
export function selectAllActivations(manifest, inputs) {
    const records = [];
    const activations = [];
 ...
 */
export function pickNearFarChunks(manifest, levelIndex, cameraPos) {
...
//# sourceMappingURL=lod-math.js.map