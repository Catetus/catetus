 ...
 * `splatforge lodge build`. Produces canonical SoA bytes + layout in the
 ...
 */
export function decodePlyToSoa(plyBytes) {
    ...
    if (header.bodyOffset + bodyBytes > plyBytes.byteLength) {
        throw new Error(`ply_invalid: body truncated (need ${bodyBytes} from offset ${header.bodyOffset}, have ${plyBytes.byteLength - header.bodyOffset})`);
    }
    const body = new DataView(plyBytes.buffer, plyBytes.byteOffset + header.bodyOffset, bodyBytes);
    // Property indices.
    const ix = (name) => {
        const i = header.props.indexOf(name);
        if (i < 0)
            throw new Error(`ply_invalid: missing required property '${name}'`);
        return i;
    };
    const idx = {
        x: ix('x'),
...
function parseHeader(bytes) {
    ...
    while (cursor < scanEnd) {
        // Locate next '\n'.
        let nl = cursor;
        while (nl < scanEnd && bytes[nl] !== 0x0a)
            nl++;
        if (nl >= scanEnd)
            throw new Error('ply_invalid: header overflowed scan window');
        ...
        if (line === 'end_header')
            break;
        if (lines.length > MAX_HEADER)
            throw new Error('ply_invalid: header too long');
    }
    ...
    if (vertexCount < 0)
        throw new Error('ply_invalid: no vertex count');
    for (const r of REQUIRED) {
        if (!props.includes(r))
            throw new Error(`ply_invalid: missing property '${r}'`);
    }
    return {
        vertexCount,
        props,
