 ...
 */
function extractAttributesFromPrimitive(prim) {
    ...
    if (isObject(primAttrs)) {
        ...
        if (hasRc) {
            ...
            const posIdx = typeof pa['POSITION'] === 'number'
                ? pa['POSITION']
 ...
 */
function gatherPrimitiveAttributes(g) {
    const out = [];
    for (const mesh of g.meshes ?? []) {
        for (const prim of mesh.primitives ?? []) {
            const got = extractAttributesFromPrimitive(prim);
 ...
 */
function selectAttributesForBuffer(g, allAttrs, bufIdx, fallback) {
    if (bufIdx === undefined)
        return fallback;
    for (const a of allAttrs) {
        const s = accessorSlice(g, a.POSITION);
        if (s && s.bufferIdx === bufIdx)
            return a;
    }
    return fallback;
}
