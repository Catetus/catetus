...
function attributionForSplatbench(scene) {
  ...
  return 'SplatForge synthetic probe (Apache-2.0, deterministic).';
}
...
function buildSplatbenchEntries(splatbench) {
  return splatbench.scenes.map((s) => {
    ...
    return {
      ...
      // SplatForge KHR-spz subset by construction (we wrote them).
...
function main() {
  ...
  const index = {
    schema: 'splatforge.explore-index/0.1',
    generatedAt: new Date().toISOString().slice(0, 10),
    splatbenchVersion: splatbench.splatforgeVersion,
