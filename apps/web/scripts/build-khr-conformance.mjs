 ...
 *   1. Asking cargo to build the catetus-khr-conformance fixture
...
function specMetadata() {
  ...
  // Bumped together with crates/catetus-khr-conformance/Cargo.toml.
  return {
...
function runBuild() {
  ...
  execFileSync('cargo', ['build', '-p', 'catetus-khr-conformance', '--quiet'], {
    cwd: REPO_ROOT,
    stdio: 'inherit',
  });
  ...
  try {
    const fixturesBin = resolve(REPO_ROOT, 'target', 'debug', 'catetus-khr-fixtures');
    execFileSync(fixturesBin, [tmp], { stdio: 'inherit' });

    const validateBin = resolve(REPO_ROOT, 'target', 'debug', 'catetus-khr-validate');
    ...
    for (const f of fixtures) {
      ...
      report.source = `crates/catetus-khr-conformance/fixtures/${f}`;
    ...
    const payload = {
      ...
      spec: specMetadata(),
      crate: { name: 'catetus-khr-conformance', version: '0.2.0' },
