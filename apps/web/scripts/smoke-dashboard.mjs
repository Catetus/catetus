 ...
 *
 * Usage:
 *   pnpm --filter @splatforge/web build      # produces dist/
 *   pnpm --filter @splatforge/web preview &  # serves on :4321
...
try {
  ...
  // ---- Test 2: with mocked API → renders all 3 sections
  {
    ...
    if (!emailText?.includes("test@splatforge.dev"))
