...
//
// Uploads a SplatForge-generated 3D Tiles tileset (with KHR_gaussian_splatting)
...
//     --name "SplatForge bonsai_real demo" \
...
function parseArgs(argv) {
  const args = {
    ...
    name: "SplatForge bonsai_real demo",
    description:
      "Live KHR_gaussian_splatting tileset emitted by SplatForge from the Mip-NeRF 360 bonsai scene (iter7000, 1.06M splats, 4-level LOD).",
    lon: -122.4194,
    lat: 37.7749,
    height: 0,
    force: false,
  ...
  if (!stat) {
    throw new Error(
      `tileset.json not found at ${tilesetJson}.\n` +
        ...
        `  target/release/splatforge optimize --preset geospatial \\\n` +
