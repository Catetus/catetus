...
Public landing page for Catetus. Static site built with [Astro](https://astro.build/) — zero client JS by default plus one tiny inline island for the preset toggle.
...
# from repo root
pnpm install
pnpm -F @catetus/web dev
```

## Build

```bash
pnpm -F @catetus/web build
...
- **Cloudflare Pages** — build cmd `pnpm -F @catetus/web build`, output `apps/web/dist`.
