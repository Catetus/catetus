...
pub trait JobStoreApi {
    ...
    /* ---------- audit (api-production-hardening) ---------- */

    /// Best-effort audit-event write. Implementations may stub this out
    /// (return `Ok(())`) on backends that haven't shipped the table yet —
    /// SQLite is the production target; Postgres adds the same shape via
    /// migration 0005_audit_events.sql.
