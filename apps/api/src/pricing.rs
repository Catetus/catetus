...
fn preset_compute_curve(preset: &str) -> (f64, f64) {
    ...
    match preset {
        ...
        // splatforge-qat-scaffold — Quant-Aware Retrain Scaffold-GS codec
        // (validated 2026-05-16). Wall-clock is dominated by the
        // Modal A100 retrain pass (~5-8 min on bonsai-scale Scaffold
        // PLYs), with negligible CPU overhead for the bit-packing +
        // constant-strip + int8/int4 quantization steps. Per-MB slope
        // is small because Scaffold PLY size tracks anchor count
        // sub-linearly with scene size — even bicycle-scale scenes
        // land in the 100-200 MB Scaffold-PLY range. The headline
        // result is the +0.172 dB MEAN PSNR GAIN at ~50% size across
        // 6 scenes (bonsai, bicycle, garden, stump, treehill,
        // flowers); reconstruction is bit-exact vs the retrained
        // Scaffold checkpoint, so the dB number is a baseline-
        // comparison delta and NOT a lossy-codec error metric.
        // Anchor: bonsai 130 MB Scaffold PLY → ~360 s end-to-end on A100.
