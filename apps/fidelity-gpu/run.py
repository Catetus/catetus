...
The image bundles:
  ...
  - Rust stable + splatforge CLI built from the pinned git tag
...
image = (
    ...
    .run_commands(
        ...
        # Splatforge clone + build (CLI release).
        ...
        "python3 /opt/splatforge/benches/synth_scenes.py /sbench/scenes",
...
app = modal.App("splatforge-fidelity-gpu", image=image)

# Persist results outside the container so we can pull them back from outside.
results_volume = modal.Volume.from_name(
    "splatforge-fidelity-results", create_if_missing=True
)
