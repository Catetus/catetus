...
- Per-camera render PSNR. The v0 schema has a separate `fidelity` block that
  ...
  `catetus fidelity`). v2 ingest deliberately keeps the two paths
...
The Modal entrypoint reuses the existing `catetus-worker` image base (Rust
toolchain + pinned `catetus` CLI) so we don't pay a fresh image build per
