...
Given a local PLY file and a list of presets, runs ``splatforge optimize``
...
The runner is intentionally CPU-only: ``splatforge optimize`` performs
...
# Presets we currently ship in the splatforge CLI. Validated against
# `crates/splatforge-optimize/src/presets.rs` at the v2 cutoff. Two of these
...
def _which_splatforge() -> str:
    """Locate the splatforge CLI binary.

    Prefers ``$SPLATFORGE_BIN``, then ``$PATH``, then the in-repo release
    build at ``target/release/splatforge``. Raises if none are usable.
    """
    env = os.environ.get("SPLATFORGE_BIN")
    if env and Path(env).exists():
        return env
    on_path = shutil.which("splatforge")
    if on_path:
        return on_path
    repo_root = Path(__file__).resolve().parents[4]
    candidate = repo_root / "target" / "release" / "splatforge"
    if candidate.exists():
        return str(candidate)
    raise RuntimeError(
        "splatforge CLI not found. Set $SPLATFORGE_BIN, install it on PATH, "
        "or run `cargo build --release -p splatforge-cli`."
    )


def _run_analyze(splatforge: str, ply: Path) -> dict[str, Any]:
    t0 = time.monotonic()
    proc = subprocess.run(
        [splatforge, "analyze", str(ply)],
        check=True,
        capture_output=True,
        text=True,
        timeout=300,
...
def _sum_output_bytes(manifest_path: Path) -> int:
    ...
    The splatforge optimize CLI emits a glTF JSON manifest with
...
def _run_optimize(
    splatforge: str, ply: Path, preset: str, out_dir: Path
) -> PresetRun:
    ...
    two-step pipeline is:
      ...
      1. ``splatforge optimize --preset <name>`` → glTF manifest + sidecar
         ``.bin`` buffers.
    ...
    try:
        subprocess.run(
            [splatforge, "optimize", "--preset", preset, "-o", str(gltf), str(ply)],
            check=True,
            capture_output=True,
            text=True,
            timeout=1800,
        ...
        subprocess.run(
            [splatforge, "convert", "--to", "spz", "-o", str(spz), str(gltf)],
            check=True,
            capture_output=True,
            text=True,
            timeout=600,
...
def measure_scene(
    ...
    *,
    ...
    splatforge = _which_splatforge()
    ply = ply.resolve()
    if not ply.exists():
        raise FileNotFoundError(ply)

    analyze = _run_analyze(splatforge, ply)
    ...
    try:
        for preset in presets:
            run = _run_optimize(splatforge, ply, preset, work_dir)
