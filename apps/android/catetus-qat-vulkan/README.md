...
Android Vulkan compute decoder for the [Catetus QAT-PLY v1](../../codec/qat-ply-c/)
...
From the Catetus root with Android Studio attached:

```bash
./gradlew :apps:android:catetus-qat-vulkan:assembleDebug
```

Or syntactic-only CMake validation (no compile):

```bash
cd apps/android/catetus-qat-vulkan
...
```kotlin
import dev.catetus.qat.QATPlyDecoder
...
The compiled `libcatetus_qat.so` (arm64-v8a, release) is ~95 KB
