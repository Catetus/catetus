//
// jni_bridge.cpp — JNI bridge from dev.splatforge.qat.QATPlyDecoder into
// the Vulkan decoder. Marshals Java arrays into native pointers and
// calls into splatforge::decode_int8 / decode_int4_packed.
//
...
Java_dev_splatforge_qat_QATPlyDecoder_nativeDecodeInt8(
    ...
    try {
        splatforge::decode_int8(q.data(), s.data(),
...
JNIEXPORT void JNICALL
Java_dev_splatforge_qat_QATPlyDecoder_nativeDecodeInt4Packed(
    JNIEnv *env, jclass /*clazz*/,
    ...
    std::vector<float> out((size_t)n_rows * n_channels);
    try {
        splatforge::decode_int4_packed(packed.data(), scale.data(),
                                       (uint32_t)n_rows, (uint32_t)n_channels,
                                       out.data());
    } catch (const std::exception &e) {
        env->ThrowNew(env->FindClass("java/lang/RuntimeException"), e.what());
