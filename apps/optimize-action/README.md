# Catetus Optimize — GitHub Action

A drop-in GitHub Action that submits the Gaussian-splat files in a pull request to
[Catetus Cloud](https://catetus-api.fly.dev), gates the PR on a compression
...
> ![Catetus fidelity](https://img.shields.io/badge/Catetus%20fidelity-78%2F100-brightgreen) Optimized 1 splat for commit `a1b2c3d`.
>
...
| --- | --- | --- | --- |
| `api-key` | yes | — | Bearer token. Set as a repo secret `CATETUS_API_KEY`. Get one at https://catetus.com. |
| `api-url` | no | `https://catetus-api.fly.dev` | Override for self-hosted Catetus Cloud deployments. |
...
- Calls to the Catetus API use `Authorization: Bearer <key>`; the GitHub
...
CATETUS_API_KEY=sk_... ./scripts/test-locally.sh
```
