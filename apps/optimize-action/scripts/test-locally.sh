#!/usr/bin/env bash
# Local smoke test for catetus/optimize-action.
#
...
#
# Hits the LIVE Fly API. Requires CATETUS_API_KEY in env.
#
# Usage:
...
: "${CATETUS_API_KEY:?CATETUS_API_KEY must be set in env}"
: "${API_URL:=https://catetus-api.fly.dev}"
...
WORK=$(mktemp -d -t catetus-action-smoke.XXXXXX)
trap 'rm -rf "$WORK"' EXIT
bold "→ workdir: $WORK"

cd "$WORK"
git init -q
git config user.email "smoke@catetus.local"
...
OUT_FILE=$(mktemp -t catetus-output.XXXXXX)
...
env -i \
  ...
  GITHUB_REPOSITORY="catetus/smoke" \
  INPUT_API_URL="$API_URL" \
  INPUT_API_KEY="$CATETUS_API_KEY" \
  INPUT_PRESET="$PRESET" \
