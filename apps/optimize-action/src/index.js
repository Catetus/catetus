#!/usr/bin/env node
// Catetus Optimize — GitHub Action entrypoint.
//
// Submits every changed splat file in the PR to Catetus Cloud, polls until
// terminal, computes a compression report, and posts a sticky PR comment with
// a fidelity badge. Designed to drop in as `catetus/optimize-action@v1`.
//
...
class CloudClient {
  ...
  _headers(extra = {}) {
    return {
      ...
      'user-agent': 'catetus-optimize-action/1.0',
      ...extra,
    };
  }
...
function badgeFor(score, ratio) {
  const color = ratio == null ? 'lightgrey'
    ...
    : ratio <= 0.7 ? 'yellow'
    : 'orange';
  const label = encodeURIComponent('Catetus fidelity');
...
function renderComment({ results, apiUrl, headSha, threshold }) {
  const lines = [];
  lines.push('<!-- catetus-optimize-action -->');
  const aggRatio = aggregateRatio(results);
  const aggScore = aggRatio == null ? null : Math.max(0, Math.min(100, 100 * (1 - aggRatio)));
  lines.push(`![Catetus fidelity](${badgeFor(aggScore, aggRatio)})`);
  lines.push('');
  lines.push(`**Catetus Cloud** optimized ${results.length} splat${results.length === 1 ? '' : 's'} for commit \`${headSha.slice(0, 7)}\`.`);
  lines.push('');
  ...
  lines.push('');
  lines.push(`<sub>via [catetus/optimize-action](https://github.com/catetus/optimize-action) — API: \`${apiUrl}\`</sub>`);
  return lines.join('\n');
}
...
async function upsertStickyComment({ token, repo, prNumber, body }) {
  ...
  const list = await request(`https://api.github.com/repos/${owner}/${name}/issues/${prNumber}/comments?per_page=100`, {
    ...
    headers: {
      ...
      'user-agent': 'catetus-optimize-action',
  ...
  const existing = JSON.parse(list.body.toString('utf8'))
    .find((c) => c.body && c.body.includes('<!-- catetus-optimize-action -->'));
  const headers = {
    'authorization': `Bearer ${token}`,
    'user-agent': 'catetus-optimize-action',
    'accept': 'application/vnd.github+json',
...
async function main() {
  const apiUrl = getInput('api-url', { defaultValue: 'https://catetus-api.fly.dev' });
  ...
  if (!apiKey) throw new Error('api-key input is required (set repo secret CATETUS_API_KEY)');
  ...
  info(`catetus/optimize-action — api=${apiUrl} preset=${preset} threshold=${threshold}`);
