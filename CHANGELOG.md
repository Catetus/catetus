...
All notable changes to Catetus are documented in this file.
...
- **Repack ΔPSNR column on SplatBench leaderboard.** A new `Repack ΔPSNR (premium)`
  column reports PSNR delta of `catetus-pro`'s differentiable repack vs.
...
- **Hosted API on Fly.** `apps/api` deployed at `https://catetus-api.fly.dev`
  (`/healthz`, `POST /v1/jobs`, `GET /v1/jobs/:id`, `POST /v1/jobs/:id/upload`).
- **TryIt component on catetus.com actually calls the live API.**
...
- **ML Score metric calibrated against empirical-p99 RMSE distribution.** The
  `catetus-pro` perceptual metric advanced from `0.1.1-baseline` to
...
- **ML Score column on SplatBench leaderboard.** A new `ML Score (pro)` column
  ...
  Astro site at `catetus.com`. Values are computed by
  `catetus-pro` — a proprietary build that scores rendered vs. baseline
...
- **SPEC-0013** (`KHR_mesh_quantization` for splat attributes) implemented. The
  ...
  per-component `min`/`max`) when `WriteOpts::quantize == true`. `catetus
  ...
  from ~5× to ~2.5×.
- **`@catetus/viewer`** learns to decode the new integer accessors.
...
- **`apps/api`** ships a `README.md` documenting the dev-mode story — a
  contributor can `cargo run -p catetus-api` from a fresh checkout and
...
- GitHub repo metadata polished: topic tags (`gaussian-splatting`, `webgpu`,
  `3d`, `splat`, `rust`, `gltf`, `catetus`, `computer-graphics`),
...
- **Visual-fidelity benchmark** — every scene in SplatBench is now measured for
  ...
  8 deterministic orbit frames through `@catetus/viewer` in headless
...
- **`@catetus/viewer` is now actually runnable** — previously the SDK was
  tsc-clean but had never been exercised against a real catetus-emitted
...
- **CLI** (`catetus`) with `analyze`, `inspect`, `convert`, `optimize`,
  `preview`, `diff`, `benchmark`, and `corpus run` subcommands.
- **`catetus-core`** — canonical `SplatIR` with deterministic serialization,
...
- **`catetus-ply`** — Inria 3DGS PLY reader (binary little-endian + ASCII)
  with structured errors and writer for round-trip (SPEC-0002).
- **`catetus-spz`** — v2 SPZ codec (24-bit fixed-point positions,
  smallest-three quaternions, 8-bit scales/colors, zlib payload) with
  round-trip parity tests (SPEC-0003).
- **`catetus-gltf`** — glTF 2.0 + `KHR_gaussian_splatting` writer with
...
- **`catetus-optimize`** — composable `Pass` framework with 10 passes
...
- **`@catetus/viewer`** — WebGPU primary + WebGL2 fallback Gaussian-splat
  ...
  (SPEC-0008).
- **`@catetus/report-ui`** — visual-diff + viewer-parity HTML templates.
...
- **End-to-end `catetus diff`** — Rust CLI dispatches to a Node helper that
...
- `catetus diff` requires `playwright-core + chromium` for the real
