...
- `splatCount`, `shDegree`, `hash` — taken from `catetus analyze` (BLAKE3
...
1. `catetus optimize --preset <name>` → glTF manifest + sidecar buffers.
...
- Per-camera render PSNR — that's the `catetus fidelity` path, which
  runs a deterministic 8-orbit camera sweep through `@catetus/viewer`
...
The Modal entrypoint reuses the existing `catetus-worker` image base
(Rust toolchain + pinned `catetus` CLI) so we don't pay a fresh image
...
path. Once they appear in `crates/catetus-optimize/src/presets.rs` the
...
(catetus-pro ML scores, DifferentiableRepack rows, ΔE94 fidelity
...
the canonical IR (computed by `catetus analyze`), so a fresh run on
a fresh machine produces byte-identical numbers when:

- The PLY input matches the recorded hash.
- The `catetus` CLI is built at the recorded git SHA (see
  `catetusVersion` at the top of the JSON).
...
schema records both `catetusVersion` and `hash` per row.
