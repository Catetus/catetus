# `catetus/optimize-action` — Launch-Week Plan

## Goal

Make `catetus/optimize-action@v1` the default way a Babylon/Three/Cesium
...
- [ ] Publish `catetus/optimize-action` repo (mirror `apps/optimize-action/`).
- [ ] Tag `v1.0.0` + create a `v1` major-version moving tag.
- [ ] Submit listing to the **GitHub Marketplace** (verified-publisher flow).
- [ ] Land a "Using the action" section in the main Catetus README that
      links here.
- [ ] Land 2 logo cards on `catetus.com` (any two of: a museum demo, an
      e-comm shop, an architecture-viz studio that's already in the design
      partner program).
- [ ] Stand up `catetus.com/optimize-action` landing page with the same
...
**Title:** `Show HN: Catetus Optimize – a GitHub Action that compresses 3D splat PRs`

**Body strawman:**

> Hi HN, I've been working on Catetus — an optimizer that turns raw Gaussian
...
> repack (paid).
>
> Code: github.com/catetus/optimize-action
> Demo screencap: <link>
> API source + design notes: github.com/catetus/catetus
>
...
> Adds [`catetus/optimize-action`](https://github.com/catetus/optimize-action),
> a GitHub Action that compresses Gaussian-splat PRs to web-ready GLB via the
> Catetus Cloud API and posts a fidelity-badge PR check. Useful for any
...
> Workflow:
> ```yaml
> - uses: catetus/optimize-action@v1
>   with:
>     api-key: ${{ secrets.CATETUS_API_KEY }}
> ```
>
> Posts a sticky comment with the fidelity badge. Free tier is 100 jobs/month.
> Source + design rationale: github.com/catetus/optimize-action
...
> your three.js / babylon.js / cesium repo, runs it through the Catetus
> optimizer, and posts a fidelity badge on the PR. Free tier covers most OSS
> repos.
>
> 2/ One liner: `uses: catetus/optimize-action@v1`
...
- Cross-link from the catetus.com homepage to top HN comment if it lands
  on the front page.
...
- Land a deeper write-up on the Catetus blog comparing PSNR/SSIM scores
...
| Unique repos using action | `gh search code "uses: catetus/optimize-action"` | 30 |
