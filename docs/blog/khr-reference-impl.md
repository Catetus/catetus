...
*Draft, May 2026 — author: Catetus engineering*
...
Three artifacts, all in the public Catetus repo
([github.com/catetus/catetus](https://github.com/catetus/catetus)):

1. **`catetus-gltf`** — the writer. ~1,400 lines of Rust that emit
...
2. **`catetus-khr-validate`** — the validator. A pure-Rust CLI that
...
All three live under `crates/catetus-khr-conformance`. To regenerate
...
byte buffers, but `catetus-khr-validate` rejects one and accepts the
other.
...
Catetus's v2 plan (24-month, 30-engineer, $5-15M ARR target) makes
...
underlying `catetus-gltf` crate is what makes us the *reference*
