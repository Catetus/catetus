...
+---------------------------------+   splatforge-optimize   splatforge-bench
                ...
                +----------------------------------------+
                  ^         ^         ^         ^
                  |         |         |         |
            splatforge-  splatforge-  splatforge-  splatforge-
...
OpenUSD (SPEC-0011 + SPEC-0012) is the v0.2 target — `splatforge-usd` already
...
* **`splatforge analyze`** produces byte-identical JSON across runs.
* **`splatforge optimize`** is deterministic given `(input, preset, seed)`.
...
| ----------------------------- | ------------ | ---------- | ------------------------------------ |
| `crates/splatforge-core`      | Rust lib     | shipped    | IR + hash + analyze report           |
| `crates/splatforge-ply`       | Rust lib     | shipped    | Inria 3DGS PLY in/out                |
| `crates/splatforge-spz`       | Rust lib     | shipped    | SPZ v2 codec                         |
| `crates/splatforge-gltf`      | Rust lib     | shipped    | glTF + KHR_gaussian_splatting        |
| `crates/splatforge-optimize`  | Rust lib     | shipped    | 10 passes, 8 presets                 |
| `crates/splatforge-bench`     | Rust lib     | shipped    | Corpus runner                        |
| `crates/splatforge-cli`       | Rust bin     | shipped    | The `splatforge` binary              |
| `crates/splatforge-usd`       | Rust lib     | draft v0.2 | OpenUSD I/O — SPEC-0011              |
...
| `apps/web`                    | Astro static | deployed   | https://splatforge-…vercel.app       |
...
Worked example — adding `splatforge-foo` as a new ingest/export format:

1. Create `crates/splatforge-foo/` with `read_foo(path) -> SplatScene` and
   `write_foo(scene, path, opts)`. Mirror the surface of `splatforge-ply` for
   ergonomic parity.
2. Add the crate to `Cargo.toml`'s `workspace.members`.
3. Wire ingest detection in `splatforge-core::format_from_extension` +
   `format_from_magic`.
4. Wire the CLI subcommand into `crates/splatforge-cli/src/main.rs` —
...
6. Add a round-trip test in `crates/splatforge-foo/tests/roundtrip.rs`.

`splatforge-usd` was added this way and is the freshest reference.

## How to add a new optimize pass

1. Implement `Pass` in `crates/splatforge-optimize/src/passes.rs`.
2. Reference it from one or more presets in `crates/splatforge-optimize/src/presets.rs`.
3. Add a unit test in `crates/splatforge-optimize/tests/passes.rs` asserting
