# Cesium ion quickstart for SplatForge `--preset geospatial`

SplatForge's `geospatial` preset emits a [Cesium 3D Tiles 1.1](https://github.com/CesiumGS/3d-tiles) tileset
...
splatforge inspect my-asset/lod0.glb
...
curl -X POST https://api.cesium.com/v1/assets \
  ...
  -d '{
        "name": "SplatForge geospatial sample",
...
Cesium handles the screen-space-error refinement automatically. The four LOD levels SplatForge emits give
...
| Refinement never picks finer LODs | Reduce `maximumScreenSpaceError` (more aggressive refinement) or check that the bounding boxes are correctly oriented. SplatForge emits axis-aligned boxes in the local frame — if the capture's local frame is offset from ECEF, set the per-tileset `transform` in CesiumJS. |
| Coarsest LOD is too low quality at intro fly-in | Increase the LOD chain depth in `splatforge-optimize/src/presets.rs` (`BuildLOD` levels). |

## 7. Sample tileset

A tiny committed-to-git reference tileset lives at
`crates/splatforge-optimize/tests/fixtures/geospatial-sample/`. It is built deterministically by the
