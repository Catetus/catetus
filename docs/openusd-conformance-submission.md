...
This document is the cover-letter for the Catetus submission to the
OpenUSD / AOUSD core spec working group, companion to the public forum
post in [`(private outreach draft)`](standards-outreach/openusd-forum-post.md).
The detailed clause table and sample output live in
[`crates/catetus-usd-conformance/conformance.md`](../crates/catetus-usd-conformance/conformance.md).

## What we are submitting

Three artifacts, all in this repository:

1. **`catetus-usd-validate`** — a pure-Rust CLI that loads a `.usda`
...
2. **An 8-file fixture corpus** under
   `crates/catetus-usd-conformance/fixtures/`. Five valid fixtures
...
3. **A conformance test matrix** (`cargo test -p catetus-usd-conformance`)
...
Every fixture is produced by the `catetus-usd-fixtures` binary, which
calls `catetus_usd::{write_usda, write_usdc, render_usda}` with seeded
...
`catetus_usd::write_usdc`'s version-0.0.1 binary encoder; the validator
decodes it via `catetus_usd::read_usdc`, re-emits canonical USDA via
...
# As a sibling-binary to the catetus CLI:
catetus spec-check crates/catetus-usd-conformance/fixtures/01_valid_minimal.usda
# (auto-detects the OpenUSD validator from the .usda/.usdc extension and shells
#  out to `catetus-usd-validate`; override the resolved binary via the
#  `CATETUS_USD_VALIDATE` env var for local dev.)
```
...
An asset is "conformant" iff `catetus-usd-validate` exits 0. Skipped
...
[`crates/catetus-usd/SPEC-GAPS.md`](../crates/catetus-usd/SPEC-GAPS.md);
...
- **SH coefficient layout** (clause `SH_COEFFS_COUNT`). 26.03 has no
  ...
  `custom float[] catetus:shCoefficients` and accept any of the four
...
[`(private outreach draft)`](standards-outreach/openusd-forum-post.md).
...
(`catetus-private/docs/engplan-prd-v2.md`) makes the OpenUSD
...
plus the underlying `catetus-usd` crate land Catetus as the first.
