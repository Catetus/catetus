...
[`crates/catetus-khr-conformance/conformance.md`](../crates/catetus-khr-conformance/conformance.md).

## What we are submitting

Three artifacts, all in this repository:

1. **`catetus-khr-validate`** — a pure-Rust CLI that loads a `.gltf` or
...
2. **A 10-file fixture corpus** under
   `crates/catetus-khr-conformance/fixtures/`. Five valid fixtures
...
3. **A conformance test matrix** (`cargo test -p catetus-khr-conformance`)
...
Every fixture is produced by the `catetus-khr-fixtures` binary, which
calls `catetus-gltf::{write_gltf, write_glb}` with seeded inputs (no
...
cargo run -p catetus-khr-conformance --bin catetus-khr-validate -- \
    crates/catetus-khr-conformance/fixtures/01_valid_baseline.glb --json
```
...
An asset is "conformant" iff `catetus-khr-validate` exits 0. Skipped
...
We have observed at least one in-the-wild writer (Catetus's own,
...
or the underlying `catetus-gltf` writer.

The v2 engineering plan
(`catetus-private/docs/engplan-prd-v2.md`) makes the Khronos submission
...
underlying `catetus-gltf` crate land Catetus as the first.
