# HAC++ Implementation Spec for SplatForge
...
7. **Compatibility with our existing PostHAC container**: we have a deployed Rust decoder for the v0.1 container (`splatforge-posthac` crate). HAC++'s bitstream is a *strict superset* of v0.1 if we version-bump. Or do we ship HAC++ as a parallel format with a different magic? Recommend the latter — HAC++ is sufficiently more complex that a forked decoder is cleaner than versioning. Magic `HCPP` (`0x50504348`).
