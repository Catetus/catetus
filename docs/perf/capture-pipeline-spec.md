...
SplatForge today is a hosted compression API: customers upload an
...
N photos.zip
  ...
  → public worker forwards to private `splatforge-capture` Modal app
...
| Private Modal app `splatforge-capture` | `splatforge-private/apps/capture/capture_modal.py` | SCAFFOLD |
...
The public worker forwards to `SPLATFORGE_CAPTURE_URL` (Modal secret;
...
```json
{
  ...
  "filename": "photos.zip",
  "callback_url": "https://api.splatforge.dev/v1/jobs/<id>/status",
  "inner_preset": "codec-gs-mixed",   // optional, default codec-gs-mixed
...
**Milestone 4**: implement `_encode_via_inner_preset()` →
  * dispatch to existing `splatforge-codec-gs-mixed` Modal app
...
**Milestone 5**: implement `_upload_artifact()` and end-to-end on a
  ...
  `SPLATFORGE_CAPTURE_URL` on the public worker.
