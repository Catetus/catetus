...
`crates/catetus-lodge/` + the `catetus lodge build` CLI subcommand
...
`catetus-ply::write_ply`. Each chunk is independently loadable by
...
`catetus lodge unpack -i <dir> --level 0 -o out.ply` reassembles
...
test in `crates/catetus-cli/tests/cli_smoke.rs::
...
catetus lodge build -i scene.ply -o scene.lodge --levels 5

# Print a summary of an existing pyramid
catetus lodge info -i scene.lodge

# Reassemble one level back into a flat PLY
catetus lodge unpack -i scene.lodge --level 0 -o roundtrip.ply
```
...
`cargo test -p catetus-lodge`: 8 / 8 pass (Phase A.1 untouched).
