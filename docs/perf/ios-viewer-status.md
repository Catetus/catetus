...
| Component                                                | Status                          |
|----------------------------------------------------------|---------------------------------|
| Rust core `splatforge-viewer-mobile`                     | builds; clippy clean            |
| `cargo test -p splatforge-viewer-mobile`                 | 3/3 pass (synth roundtrip, depth sort, optional bonsai) |
...
| `SplatforgeViewerCore.xcframework` (5-slice)             | builds via `scripts/build-xcframework.sh` |
...
1. **No `.xcodeproj`** — the SwiftUI demo is declared as a SwiftPM `.library`
   ...
   - choose `File → New → Project → iOS App`, drop the `SplatforgeViewer`
...
4. **Wire `RadixSort.metal` into the frame loop**, replacing
   ...
   is allocating the histogram + ping-pong buffers in `SplatforgeRenderer`
...
# 4. Validate.
cargo test -p splatforge-viewer-mobile
(cd packages/viewer-mobile/ios && swift test)
(cd packages/viewer-mobile/ios && \
 xcodebuild -scheme SplatforgeViewer -destination 'generic/platform=iOS' build)
...
# add SplatforgeViewer SPM dep, copy ContentView from iOSDemoApp.swift,
...
renderers. SplatForge already has the **best public open-source web viewer**
