# Getting started with SplatForge
...
```bash
splatforge analyze fixtures/tiny/basic_binary.ply
```

This prints a deterministic JSON report. To save it:

```bash
splatforge analyze fixtures/tiny/basic_binary.ply --pretty > analyze.json
```

## 3. Optimize for the web

```bash
splatforge optimize fixtures/tiny/basic_binary.ply \
  --preset web-mobile \
  --out out/scene.gltf
```
...
```bash
splatforge preview out/scene.gltf
...
```bash
splatforge diff fixtures/tiny/basic_binary.ply out/scene.gltf \
  --threshold 0.03 \
  --out reports/diff/
...
If `playwright-core` is missing, `splatforge diff` still produces a valid
...
You can override the helper location with `SPLATFORGE_DIFF_HELPER=/path/to/diff-cli.mjs`
(useful when running an installed binary outside the source tree). Set
`SPLATFORGE_INCLUDE_TIMESTAMPS=1` to embed a `generatedAt` field — the default
output is deterministic so snapshot tests stay stable.

## 6. Run a parity matrix

```bash
pnpm -F splatforge-visual run test
