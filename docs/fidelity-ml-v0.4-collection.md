...
`fidelity-ml v0.3` (shipped in `crates/catetus-fidelity-ml/data/weights-v0.3.json`)
...
The public page lives at **`catetus.com/rate`** and is wired to the
...
Catetus readers know what compression artifacts look like — they're
...
* The Bradley-Terry trainer lives in the private repo
  (`catetus-pro/training/fidelity-ml-v0.4/`) and pulls `/v1/ratings/summary`
  on a cron. Public repo does not host the trainer because the v0.4 weights
  are part of `catetus-pro`, the same as v0.3.
