...
**Status:** Draft / Release-Candidate (Catetus implementation).
...
**Maintainer:** Catetus (`crates/catetus-gltf`).
...
extension Catetus ships (alongside the lossless `KHR_gaussian_splatting`
...
`crates/catetus-spz`), a vendor-extension byte (used by Adobe), and a
...
- **`version`** (integer, required) — SPZ wire-format version. Currently
  `2` (matches `SPZ_VERSION` in `crates/catetus-spz`). Readers MUST
...
extension is silent on that choice. Catetus's writer emits
...
`catetus-spz::encode_spz` returns, and what `catetus-spz::read_spz`
accepts. No padding, no length prefix added on top of the SPZ bytes.

## 5. Conformance clauses

The validator (`crates/catetus-khr-conformance`) implements five
...
## 6. Writer behavior (Catetus)

`catetus-gltf` exposes the extension via `WriteOpts.compress`:
...
1. The scene is encoded with `catetus-spz::encode_spz` to produce the
   blob.
...
`catetus-gltf::read_glb` detects the SPZ extension on the primitive,
slices the SPZ blob out of buffer 0 using the declared `bufferView`,
and dispatches to `catetus-spz::read_spz_bytes`. The result is a
...
- **Should fallback accessor population be normative?** Catetus ships
...
*This document, the writer in `crates/catetus-gltf`, the validator
clauses in `crates/catetus-khr-conformance`, and the fixture corpus
together constitute Catetus's reference implementation of the
extension. See `(private outreach draft)` for the
