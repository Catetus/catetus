 ...
 * SplatForge's glTF are different formats — and SplatForge's viewer doesn't
 ...
 * SplatForge baseline render (lossless-repack of the same source PLY)
 * using the same ΔE94 / SSIM / pixelmatch math `tests/visual/scripts/splatbench-fidelity.mjs`
 * already uses for SplatForge's own presets.
 *
 * CLI:
 *   node score.mjs --sog <output.sog> --ply <source.ply> --out <metrics.json> [opts]
 *
 * Options:
 *   --baseline <dir>      Directory of pre-rendered SplatForge baseline PNGs (0000..0007.png).
...
const SPLATFORGE_CLI = resolve(ROOT, 'target/release/splatforge');

if (!existsSync(SOG_LOADER)) {
  console.error(`[score] missing ${SOG_LOADER}. Run \`pnpm -F @splatforge/viewer build\` first.`);
  process.exit(2);
}
...
/* numbers land on the same axis as the SplatForge-preset numbers.        */
/* --------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------- */
/* Baseline (SplatForge lossless-repack) — only runs when --baseline is   */
...
function optimize(ply, preset, outDir) {
  ...
  const res = spawnSync(SPLATFORGE_CLI, ['optimize', ply, '--preset', preset, '--out', gltf], {
    stdio: 'inherit',
  });
  if (res.status !== 0) throw new Error(`splatforge optimize ${ply} exited ${res.status}`);
  return gltf;
}

function inputPlyBbox(ply) {
  const res = spawnSync(SPLATFORGE_CLI, ['analyze', ply], { encoding: 'utf8' });
  ...
  try {
    ...
    try {
      if (args.baseline && existsSync(args.baseline)) {
        ...
        if (!existsSync(SPLATFORGE_CLI)) {
          throw new Error(
            `splatforge CLI missing at ${SPLATFORGE_CLI}; supply --baseline <dir> instead`,
          );
        }
        console.error('[score] rendering SplatForge baseline (lossless-repack)…');
      ...
      const out = {
        schema: 'splatforge.splatbench.encoder-fidelity/0.1',
        sog: args.sog,
        ply: args.ply,
