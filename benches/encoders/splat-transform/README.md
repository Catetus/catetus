...
camera path SplatForge uses on its own outputs — see
...
- splat-transform doesn't surface a fidelity report; the column will show
  compression-only until we pipe the SOG through SplatForge's WebGL2
  viewer (which can read SOG via the `splatforge-spz` codepath).
