#!/usr/bin/env bash
# Bench runner for the SplatForge QAT codec on Scaffold-GS PLYs.
#
...
#
# This invokes the hosted `splatforge-qat-scaffold` codec — same endpoint a
...
#
# If `SPLATFORGE_QAT_ENDPOINT` is unset, falls back to a pre-staged manifest
...
if [[ -n "${SPLATFORGE_QAT_ENDPOINT:-}" ]]; then
    start=$(date +%s.%N)
    curl -fsSL \
        -H "Content-Type: application/octet-stream" \
        -H "Authorization: Bearer ${SPLATFORGE_QAT_TOKEN:-anon}" \
        --data-binary @"$INPUT_PLY" \
        -o "$output_file" \
        "${SPLATFORGE_QAT_ENDPOINT%/}/v1/qat-scaffold" \
        > "$log_file" 2>&1
    end=$(date +%s.%N)
    ...
    echo "qat-scaffold-gs: need SPLATFORGE_QAT_ENDPOINT or PRE_STAGED_DIR" >&2
    exit 1
fi
