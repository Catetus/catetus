# Catetus real-world demo — Mip-NeRF360 `bonsai`
...
**Run date:** 2026-05-14
**Catetus build:** `catetus 0.1.0` (release, aarch64-linux), commit `HEAD` of this repo.
...
`catetus analyze` ran in **1.24 s wall** (1.06 s CPU, 99% utilization) with 608 MB peak RSS. It auto-recommended the `web-mobile` preset because the input is larger than 100 MB.
...
`catetus analyze` produces a stable BLAKE3 hash regardless of run count or wall-clock. Two consecutive runs of `catetus analyze bonsai.ply` produced byte-identical JSON output, hash `blake3:26d535…`. Optimization output is similarly deterministic given the same preset + version.

## 5. Visual fidelity (added in v0.1.1)

Eight deterministic orbit-8 frames rendered through `@catetus/viewer` in headless Chromium (SwiftShader software WebGL2, 512×512), then compared to the `lossless-repack` baseline using CIE ΔE94 / pixelmatch / per-block SSIM.
...
# 3. Analyze
./target/release/catetus analyze bonsai.ply --pretty > analyze.json

# 4. Optimize and convert across presets
for P in lossless-repack web-desktop web-mobile quest-browser size-min; do
  ./target/release/catetus optimize bonsai.ply --preset $P --out out/$P/scene.gltf
  ./target/release/catetus convert out/$P/scene.gltf --to spz --out out/$P/scene.spz
done
```

## 7. What this proves

* **Catetus can ingest a real production-scale Gaussian Splat (1.16M splats, 273 MB) and produce a 12.0 MB streamable web-mobile delivery package in under 1 second.**
