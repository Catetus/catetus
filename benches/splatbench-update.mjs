...
// Optional — only present once `catetus-pro` runs differentiable repack on
...
splatbench.catetusVersion = VERSION;
...
for (const scene of splatbench.scenes) {
  ...
  const presetSummary = (presetName) => {
    ...
    if (m.mlScore) {
      // catetus-pro splat-aware perceptual metric (proprietary; private
      // companion build). The metric values land in the public benchmark so
      // visitors can read them, but only `catetus-pro` reproduces them.
  ...
  // catetus-pro DifferentiableRepack — proprietary premium-tier result.
...
html = html.replace(
  /class="sub">[^<]*</,
  'class="sub">Initial benchmark — 7 scenes, 3 presets, 3DGS-format-only. v0.1.1 adds visual fidelity (ΔE94 / pixelmatch / SSIM) via @catetus/viewer headless renders.<',
);
...
md = md.replace(/\*\*Catetus version:\*\* `0\.1\.0`/, `**Catetus version:** \`${VERSION}\``);
